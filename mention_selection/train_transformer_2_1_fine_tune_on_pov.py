...
from modeling_bert_coref_2_1 import BertForSequenceClassificationCoref
...
class POVTransformer(object):
    def __init__(self, mode):
        ...
        #self.tokenizer = BertTokenizer.from_pretrained('pretrained_models_3_6/', do_basic_tokenize=False)
    ...
    def get_batch(self, training_data_path, index):
        ...
        first_mention_bit_seql_list = data_set[6]
        second_mention_bit_seql_list = data_set[7]
        focus_mention_length_list = data_set[8]
        other_mention_length_list = data_set[9]
        coreference_chain_length_list = data_set[10]
        used_bit_seql_1_list = data_set[11]
        same_bit_seql_1_list = data_set[12]
        used_bit_seql_2_list = data_set[13]
        same_bit_seql_2_list = data_set[14]
        label_list = data_set[15]
    ...
    def get_updated_sequence(self, raw_sequence, related_mentions, mention_parts=None, focus_mention_len=-1, offset=0, related_mention_cluster_ids=None, focus_mention_cluster_id=-100):
        ...
        related_offset = related_offset + offset
            ...
            for data_path in files:
                ...
                for raw_document_data in self.raw_document_data_list:
                    ...
                    for raw_sequence_data in raw_sequence_data_list:
                        ...
                        if len(mentions_to_replace) == 0:
                            ...
                            for i in range(len(mention_set)):
                                ...
                                if mention.lower() != focus_mention.lower():                                
                                    ...
                                    focus_mention_length_list.append(len(focus_mention_parts))
                                    other_mention_length_list.append(len(focus_mention_parts))
                                    coreference_chain_length_list.append(focus_mention_cluster_total_mention)
                        
                                    used_bit_seql_1_list.append(used_bit_focus)
                                    same_bit_seql_1_list.append(same_bit_focus)
                                    used_bit_seql_2_list.append(used_bit_focus)
                                    same_bit_seql_2_list.append(same_bit_focus)
                            ...
                            for i in range(len(related_mentions)):      
                                ...
                                if related_mention[0] > 100:
                                    from_index = i - 1
                                    break
                            if from_index == -1 and related_mentions[len(related_mentions)-1][0] < 100:
                                from_index = len(related_mentions)-1
                        ...
                        first_mention_bit_seql = torch.tensor(first_mention_bit_seql_list).unsqueeze(1)
                        first_mention_bit_seql = first_mention_bit_seql.to(self.device)
                   
                        second_mention_bit_seql = torch.tensor(second_mention_bit_seql_list).unsqueeze(1)
                        second_mention_bit_seql = second_mention_bit_seql.to(self.device)
                    
                        focus_mention_length_list = self.get_mention_distance_embedding(focus_mention_length_list)
                        focus_mention_length = torch.tensor(focus_mention_length_list)
                        focus_mention_length = focus_mention_length.to(self.device)
                        
                        other_mention_length_list = self.get_mention_distance_embedding(other_mention_length_list)
                        other_mention_length = torch.tensor(other_mention_length_list)
                        other_mention_length = other_mention_length.to(self.device)

                        coreference_chain_length_list = self.get_coreference_chain_distance_embedding(coreference_chain_length_list)
                        coreference_chain_length = torch.tensor(coreference_chain_length_list)
                        coreference_chain_length = coreference_chain_length.to(self.device)
                        
                        used_bit_seql_1 = torch.tensor(used_bit_seql_1_list).unsqueeze(1)
                        used_bit_seql_1 = used_bit_seql_1.to(self.device)

                        same_bit_seql_1 = torch.tensor(same_bit_seql_1_list).unsqueeze(1)
                        same_bit_seql_1 = same_bit_seql_1.to(self.device)
                   
                        used_bit_seql_2 = torch.tensor(used_bit_seql_2_list).unsqueeze(1)
                        used_bit_seql_2 = used_bit_seql_2.to(self.device)
                    
                        same_bit_seql_2 = torch.tensor(same_bit_seql_2_list).unsqueeze(1)
                        same_bit_seql_2 = same_bit_seql_2.to(self.device)
                    
                        outputs = model(input_ids_1=input_ids_1, input_ids_2=input_ids_2, Y_1=Y_1, Y_2=Y_2, attention_mask_1=attention_mask_1, attention_mask_2=attention_mask_2, labels=labels, mention_pos_list_1=mapped_mention_pos_list_1, mention_pos_list_2=mapped_mention_pos_list_2, first_mention_bit_seql=first_mention_bit_seql, second_mention_bit_seql=second_mention_bit_seql, focus_mention_length=focus_mention_length, other_mention_length=other_mention_length, coreference_chain_length=coreference_chain_length, used_bit_seql_1=used_bit_seql_1, same_bit_seql_1=same_bit_seql_1, used_bit_seql_2=used_bit_seql_2, same_bit_seql_2=same_bit_seql_2, margin=margin)
                        loss = outputs[0]
                        output1 = outputs[1]
                        output2 = outputs[2]
                    ...
                    for raw_sequence_data in raw_sequence_data_list:  
                        ...
                        if not gold_setting:
                            ...
                            if is_identified_mention == 0 and is_annotated_mention == 1:
                                ...
                                if focus_mention_cluster_id not in cluster_id_to_used_mentions:
                                    temp = []
                                    temp.append(annotated_converted_mention.lower())
                                    cluster_id_to_used_mentions[focus_mention_cluster_id] = temp
                                else:
                                    temp = cluster_id_to_used_mentions[focus_mention_cluster_id]
                                    if annotated_converted_mention.lower() not in temp:
                                        temp.append(annotated_converted_mention.lower())
                                        cluster_id_to_used_mentions[focus_mention_cluster_id] = temp
                                if focus_mention_cluster_id not in cluster_id_to_last_mention:
                                    cluster_id_to_last_mention[focus_mention_cluster_id] = annotated_converted_mention.lower()
                                else:
                                    temp = cluster_id_to_last_mention[focus_mention_cluster_id]
                                    if temp != annotated_converted_mention.lower():
                                        cluster_id_to_last_mention[focus_mention_cluster_id] = annotated_converted_mention.lower()
                        ...
                        if len(mentions_to_replace) == 0:
                            ...
                            for i in range(len(mention_set)):
                                ...
                                if mention.lower() != converted_focus_mention.lower():                                                                   
                                    ...
                                    focus_mention_length_list.append(len(converted_focus_mention_parts))
                                    other_mention_length_list.append(len(converted_focus_mention_parts))
                                    coreference_chain_length_list.append(focus_mention_cluster_total_mention)
    ...
    def evaluate(self, model, FLAGS):
        ...
        output_file = 'result_transformer_3_6' + ".txt"
        f_output = open(output_file, 'w+', encoding='utf-8')
        f_temp_gold = open('temp_gold_transformer_3_6.txt', 'w+', encoding='utf-8')
        f_temp_test = open('temp_test_transformer_3_6.txt', 'w+', encoding='utf-8')
        file_name_gold = 'output_transformer_gold_3_6.txt'
        f_gold = open(file_name_gold, 'w+', encoding='utf-8')
        file_name_test = 'output_transformer_auto_3_6.txt'
        ...
        sys.stdout.flush()
        valid_losses = []                
        print ('Accuracy of the trained model on the CoNLL test data %f' % (1.0*total_correct_dev / total_size_dev))
        sys.stdout.flush()
        print("total correct: ")
        sys.stdout.flush()
        ...
        print('correct mention: ')
        sys.stdout.flush()
        print(correct_mention)
        sys.stdout.flush()
        print('correct verb: ')
        sys.stdout.flush()
        print(correct_verb)
        ...
        sys.stdout.flush()
        print('total annotated verb: ')
        sys.stdout.flush()
        print(total_annotated_verb)
        sys.stdout.flush()
        print('total identified mention: ')
        sys.stdout.flush()
        print(identified_mention_count)
        sys.stdout.flush()
        print('total identified verb: ')
        sys.stdout.flush()
        print(identified_verb_count)  
        sys.stdout.flush()
    ...
    def train_and_evaluate(self, FLAGS):       
        ...
        #model = BertForSequenceClassificationCoref.from_pretrained('pretrained_models_3_6/')
        model = BertForSequenceClassificationCoref.from_pretrained('bert-base-uncased')  
        new_parameter_names = ['linear1.weight', 'linear1.bias', 'linear2.weight', 'linear2.bias', 
                               'bert.encoder.layer.0.attention.self.coref_value.weight', 'bert.encoder.layer.0.attention.self.coref_value.bias', 
                               'bert.encoder.layer.1.attention.self.coref_value.weight', 'bert.encoder.layer.1.attention.self.coref_value.bias', 
                               'bert.encoder.layer.2.attention.self.coref_value.weight', 'bert.encoder.layer.2.attention.self.coref_value.bias', 
                               'bert.encoder.layer.3.attention.self.coref_value.weight', 'bert.encoder.layer.3.attention.self.coref_value.bias', 
                               'bert.encoder.layer.4.attention.self.coref_value.weight', 'bert.encoder.layer.4.attention.self.coref_value.bias', 
                               'bert.encoder.layer.5.attention.self.coref_value.weight', 'bert.encoder.layer.5.attention.self.coref_value.bias', 
                               'bert.encoder.layer.6.attention.self.coref_value.weight', 'bert.encoder.layer.6.attention.self.coref_value.bias', 
                               'bert.encoder.layer.7.attention.self.coref_value.weight', 'bert.encoder.layer.7.attention.self.coref_value.bias', 
                               'bert.encoder.layer.8.attention.self.coref_value.weight', 'bert.encoder.layer.8.attention.self.coref_value.bias', 
                               'bert.encoder.layer.9.attention.self.coref_value.weight', 'bert.encoder.layer.9.attention.self.coref_value.bias', 
                               'bert.encoder.layer.10.attention.self.coref_value.weight', 'bert.encoder.layer.10.attention.self.coref_value.bias',
                               'bert.encoder.layer.11.attention.self.coref_value.weight', 'bert.encoder.layer.11.attention.self.coref_value.bias',                
                               'bert.encoder.layer.0.attention.self.coref_key.weight', 'bert.encoder.layer.0.attention.self.coref_key.bias', 
                               'bert.encoder.layer.1.attention.self.coref_key.weight', 'bert.encoder.layer.1.attention.self.coref_key.bias', 
                               'bert.encoder.layer.2.attention.self.coref_key.weight', 'bert.encoder.layer.2.attention.self.coref_key.bias', 
                               'bert.encoder.layer.3.attention.self.coref_key.weight', 'bert.encoder.layer.3.attention.self.coref_key.bias', 
                               'bert.encoder.layer.4.attention.self.coref_key.weight', 'bert.encoder.layer.4.attention.self.coref_key.bias', 
                               'bert.encoder.layer.5.attention.self.coref_key.weight', 'bert.encoder.layer.5.attention.self.coref_key.bias', 
                               'bert.encoder.layer.6.attention.self.coref_key.weight', 'bert.encoder.layer.6.attention.self.coref_key.bias', 
                               'bert.encoder.layer.7.attention.self.coref_key.weight', 'bert.encoder.layer.7.attention.self.coref_key.bias', 
                               'bert.encoder.layer.8.attention.self.coref_key.weight', 'bert.encoder.layer.8.attention.self.coref_key.bias', 
                               'bert.encoder.layer.9.attention.self.coref_key.weight', 'bert.encoder.layer.9.attention.self.coref_key.bias', 
                               'bert.encoder.layer.10.attention.self.coref_key.weight', 'bert.encoder.layer.10.attention.self.coref_key.bias',
                               'bert.encoder.layer.11.attention.self.coref_key.weight', 'bert.encoder.layer.11.attention.self.coref_key.bias',
                               'bert.encoder.layer.0.attention.self.coref_query.weight', 'bert.encoder.layer.0.attention.self.coref_query.bias', 
                               'bert.encoder.layer.1.attention.self.coref_query.weight', 'bert.encoder.layer.1.attention.self.coref_query.bias', 
                               'bert.encoder.layer.2.attention.self.coref_query.weight', 'bert.encoder.layer.2.attention.self.coref_query.bias', 
                               'bert.encoder.layer.3.attention.self.coref_query.weight', 'bert.encoder.layer.3.attention.self.coref_query.bias', 
                               'bert.encoder.layer.4.attention.self.coref_query.weight', 'bert.encoder.layer.4.attention.self.coref_query.bias', 
                               'bert.encoder.layer.5.attention.self.coref_query.weight', 'bert.encoder.layer.5.attention.self.coref_query.bias', 
                               'bert.encoder.layer.6.attention.self.coref_query.weight', 'bert.encoder.layer.6.attention.self.coref_query.bias', 
                               'bert.encoder.layer.7.attention.self.coref_query.weight', 'bert.encoder.layer.7.attention.self.coref_query.bias', 
                               'bert.encoder.layer.8.attention.self.coref_query.weight', 'bert.encoder.layer.8.attention.self.coref_query.bias', 
                               'bert.encoder.layer.9.attention.self.coref_query.weight', 'bert.encoder.layer.9.attention.self.coref_query.bias', 
                               'bert.encoder.layer.10.attention.self.coref_query.weight', 'bert.encoder.layer.10.attention.self.coref_query.bias',
                               'bert.encoder.layer.11.attention.self.coref_query.weight', 'bert.encoder.layer.11.attention.self.coref_query.bias',
                               'bert.encoder.layer.0.attention.self.coref_query0.weight', 'bert.encoder.layer.0.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.1.attention.self.coref_query0.weight', 'bert.encoder.layer.1.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.2.attention.self.coref_query0.weight', 'bert.encoder.layer.2.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.3.attention.self.coref_query0.weight', 'bert.encoder.layer.3.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.4.attention.self.coref_query0.weight', 'bert.encoder.layer.4.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.5.attention.self.coref_query0.weight', 'bert.encoder.layer.5.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.6.attention.self.coref_query0.weight', 'bert.encoder.layer.6.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.7.attention.self.coref_query0.weight', 'bert.encoder.layer.7.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.8.attention.self.coref_query0.weight', 'bert.encoder.layer.8.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.9.attention.self.coref_query0.weight', 'bert.encoder.layer.9.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.10.attention.self.coref_query0.weight', 'bert.encoder.layer.10.attention.self.coref_query0.bias',
                               'bert.encoder.layer.11.attention.self.coref_query0.weight', 'bert.encoder.layer.11.attention.self.coref_query0.bias', 
                               'bert.encoder.layer.0.attention.self.coref_query1.weight', 'bert.encoder.layer.0.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.1.attention.self.coref_query1.weight', 'bert.encoder.layer.1.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.2.attention.self.coref_query1.weight', 'bert.encoder.layer.2.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.3.attention.self.coref_query1.weight', 'bert.encoder.layer.3.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.4.attention.self.coref_query1.weight', 'bert.encoder.layer.4.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.5.attention.self.coref_query1.weight', 'bert.encoder.layer.5.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.6.attention.self.coref_query1.weight', 'bert.encoder.layer.6.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.7.attention.self.coref_query1.weight', 'bert.encoder.layer.7.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.8.attention.self.coref_query1.weight', 'bert.encoder.layer.8.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.9.attention.self.coref_query1.weight', 'bert.encoder.layer.9.attention.self.coref_query1.bias', 
                               'bert.encoder.layer.10.attention.self.coref_query1.weight', 'bert.encoder.layer.10.attention.self.coref_query1.bias',
                               'bert.encoder.layer.11.attention.self.coref_query1.weight', 'bert.encoder.layer.11.attention.self.coref_query1.bias',
                               'bert.encoder.layer.0.attention.self.coref_query2.weight', 'bert.encoder.layer.0.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.1.attention.self.coref_query2.weight', 'bert.encoder.layer.1.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.2.attention.self.coref_query2.weight', 'bert.encoder.layer.2.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.3.attention.self.coref_query2.weight', 'bert.encoder.layer.3.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.4.attention.self.coref_query2.weight', 'bert.encoder.layer.4.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.5.attention.self.coref_query2.weight', 'bert.encoder.layer.5.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.6.attention.self.coref_query2.weight', 'bert.encoder.layer.6.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.7.attention.self.coref_query2.weight', 'bert.encoder.layer.7.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.8.attention.self.coref_query2.weight', 'bert.encoder.layer.8.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.9.attention.self.coref_query2.weight', 'bert.encoder.layer.9.attention.self.coref_query2.bias', 
                               'bert.encoder.layer.10.attention.self.coref_query2.weight', 'bert.encoder.layer.10.attention.self.coref_query2.bias',
                               'bert.encoder.layer.11.attention.self.coref_query2.weight', 'bert.encoder.layer.11.attention.self.coref_query2.bias',
                               'bert.encoder.layer.0.attention.self.distance_embedding.weight',  
                               'bert.encoder.layer.1.attention.self.distance_embedding.weight', 
                               'bert.encoder.layer.2.attention.self.distance_embedding.weight', 
                               'bert.encoder.layer.3.attention.self.distance_embedding.weight', 
                               'bert.encoder.layer.4.attention.self.distance_embedding.weight', 
                               'bert.encoder.layer.5.attention.self.distance_embedding.weight',  
                               'bert.encoder.layer.6.attention.self.distance_embedding.weight', 
                               'bert.encoder.layer.7.attention.self.distance_embedding.weight',  
                               'bert.encoder.layer.8.attention.self.distance_embedding.weight',  
                               'bert.encoder.layer.9.attention.self.distance_embedding.weight', 
                               'bert.encoder.layer.10.attention.self.distance_embedding.weight',
                               'bert.encoder.layer.11.attention.self.distance_embedding.weight']
        is_pplm = FLAGS.pplm
        if is_pplm == 1:
            for name, param in model.named_parameters():
                if name not in new_parameter_names:
        ...
        for i in range(num_epochs):
            ...
            while s < 1959:
            #while s < 1021:                
                text_batch_1, batch_Y_1, mention_pos_list_1, text_batch_2, batch_Y_2, mention_pos_list_2, first_mention_bit_seql_list, second_mention_bit_seql_list, focus_mention_length_list, other_mention_length_list, coreference_chain_length_list, used_bit_seql_1_list, same_bit_seql_1_list, used_bit_seql_2_list, same_bit_seql_2_list, batch_label = self.get_batch(training_data_path, s+1)
                for j in range(1):
                    optimizer.zero_grad()
                    t_text_batch_1 = text_batch_1[j*16:j*16+16]
                    t_text_batch_2 = text_batch_2[j*16:j*16+16]
                    t_mention_pos_list_1 = mention_pos_list_1[j*16:j*16+16]
                    t_mention_pos_list_2 = mention_pos_list_2[j*16:j*16+16]
                # t_text_batch_1 = text_batch_1
                # t_text_batch_2 = text_batch_2
                # t_mention_pos_list_1 = mention_pos_list_1
                # t_mention_pos_list_2 = mention_pos_list_2
                    ...
                    t_batch_label = batch_label[j*16:j*16+16]
                    #t_batch_label = batch_label
                    labels = torch.tensor(t_batch_label).unsqueeze(0)           
                    #labels = torch.tensor(t_batch_label)
                    labels = labels.to(self.device)
                   
                    t_first_mention_bit_seql_list = first_mention_bit_seql_list[j*16:j*16+16]
                    first_mention_bit_seql = torch.tensor(t_first_mention_bit_seql_list).unsqueeze(1)
                    first_mention_bit_seql = first_mention_bit_seql.to(self.device)
                    t_second_mention_bit_seql_list = second_mention_bit_seql_list[j*16:j*16+16]
                    second_mention_bit_seql = torch.tensor(t_second_mention_bit_seql_list).unsqueeze(1)
                    second_mention_bit_seql = second_mention_bit_seql.to(self.device)
                    t_focus_mention_length_list = focus_mention_length_list[j*16:j*16+16]
                    t_focus_mention_length_list = self.get_mention_distance_embedding(t_focus_mention_length_list)
                    focus_mention_length = torch.tensor(t_focus_mention_length_list)
                    focus_mention_length = focus_mention_length.to(self.device)
                    t_other_mention_length_list = other_mention_length_list[j*16:j*16+16]
                    t_other_mention_length_list = self.get_mention_distance_embedding(t_other_mention_length_list)
                    other_mention_length = torch.tensor(t_other_mention_length_list)
                    other_mention_length = other_mention_length.to(self.device)
                    t_coreference_chain_length_list = coreference_chain_length_list[j*16:j*16+16]
                    t_coreference_chain_length_list = self.get_coreference_chain_distance_embedding(t_coreference_chain_length_list)
                    coreference_chain_length = torch.tensor(t_coreference_chain_length_list)
                    coreference_chain_length = coreference_chain_length.to(self.device)
                    t_used_bit_seql_1_list = used_bit_seql_1_list[j*16:j*16+16]
                    used_bit_seql_1 = torch.tensor(t_used_bit_seql_1_list).unsqueeze(1)
                    used_bit_seql_1 = used_bit_seql_1.to(self.device)
                    t_same_bit_seql_1_list = same_bit_seql_1_list[j*16:j*16+16]
                    same_bit_seql_1 = torch.tensor(t_same_bit_seql_1_list).unsqueeze(1)
                    same_bit_seql_1 = same_bit_seql_1.to(self.device)
                    t_used_bit_seql_2_list = used_bit_seql_2_list[j*16:j*16+16]
                    used_bit_seql_2 = torch.tensor(t_used_bit_seql_2_list).unsqueeze(1)
                    used_bit_seql_2 = used_bit_seql_2.to(self.device)
                    t_same_bit_seql_2_list = same_bit_seql_2_list[j*16:j*16+16]
                    same_bit_seql_2 = torch.tensor(t_same_bit_seql_2_list).unsqueeze(1)
                    same_bit_seql_2 = same_bit_seql_2.to(self.device)
                    
                    t_batch_Y_1 = batch_Y_1[j*16:j*16+16]
                    #t_batch_Y_1 = batch_Y_1
                    t_batch_Y_1 = self.create_padding(t_batch_Y_1,dic_list_1_1, dic_list_2_1)
                    Y_1 = torch.tensor(t_batch_Y_1)
                    Y_1 = Y_1.to(self.device)
                    
                    t_batch_Y_2 = batch_Y_2[j*16:j*16+16]
                    #t_batch_Y_2 = batch_Y_2
                    ...
                    running_loss = running_loss + loss.item()
                        ...
                        #for name, param in model.named_parameters():
                                ...
                                #if name == 'bert.encoder.layer.0.attention.self.query.bias':
                                #if name == 'bert.encoder.layer.0.attention.self.key.weight':
                                #if name == 'bert.encoder.layer.0.attention.self.key.bias':
                                #if name == 'bert.encoder.layer.0.attention.self.value.weight':
                                #if name == 'bert.encoder.layer.0.attention.self.value.bias':
                                #if name == 'bert.encoder.layer.0.attention.output.dense.weight':
                                #if name == 'bert.encoder.layer.0.attention.output.dense.bias':
                                #if name == 'bert.encoder.layer.0.attention.output.LayerNorm.weight':
                                #if name == 'bert.encoder.layer.0.attention.output.LayerNorm.bias':
                                #if name == 'bert.encoder.layer.0.intermediate.dense.weight':
                                #if name == 'bert.encoder.layer.0.intermediate.dense.bias':
                                #if name == 'bert.encoder.layer.0.output.dense.weight':
                                #if name == 'bert.encoder.layer.0.output.dense.bias':
                                #if name == 'bert.encoder.layer.0.output.LayerNorm.weight':
                                #if name == 'bert.encoder.layer.0.output.LayerNorm.bias':
                                    #print(name)
                                    #print(param.grad)
                    ...
                    del t_first_mention_bit_seql_list
                    del first_mention_bit_seql
                    del t_second_mention_bit_seql_list
                    del second_mention_bit_seql                    
                    del t_focus_mention_length_list
                    del focus_mention_length
                    del t_other_mention_length_list
                    del other_mention_length
                    del t_coreference_chain_length_list
                    del coreference_chain_length
                    del t_used_bit_seql_1_list
                    del used_bit_seql_1
                    del t_same_bit_seql_1_list
                    del same_bit_seql_1
                    del t_used_bit_seql_2_list
                    del used_bit_seql_2
                    del t_same_bit_seql_2_list
                    del same_bit_seql_2
                    del Y_1
                    del Y_2
                    del outputs
                ...
                # if (s+1) % 100 == 0:
                #     summary_writer.add_scalar('training loss', running_loss / 100, i*32+(s+1)/100)
                #     running_loss = 0.0
                ...
                del batch_label
                del first_mention_bit_seql_list
                del second_mention_bit_seql_list 
                del focus_mention_length_list 
                del other_mention_length_list
                del coreference_chain_length_list 
                del used_bit_seql_1_list 
                del same_bit_seql_1_list 
                del used_bit_seql_2_list 
                del same_bit_seql_2_list
                gc.collect()
                
                s = s+1

            print('training loss: %f' % (1.0*running_loss/1959))
            sys.stdout.flush()
            
            summary_writer.add_scalar('training loss', 1.0*running_loss/1959, i+1)
             
            model_path = "saved_models_pov_1/model_epoch_"+str(i+1)+".pt"
            ...
            # valid_losses, total_correct_dev, total_size_dev = self.get_model_development_data(development_data_dir, model, margin, dropout_rate)
            # valid_loss = numpy.average(valid_losses)       
            # print_msg = (f'dev loss: {valid_loss:.5f}')
            # print(print_msg)   
            # sys.stdout.flush()
            # valid_losses = []
            ...
            # valid_losses, total_size, total_correct, total_annotated_mention, total_annotated_verb, annotated_mention_correctly_identified_incorrectly_converted_count, annotated_mention_incorrectly_identified_count, annotated_verb_correctly_identified_incorrectly_converted_count, annotated_verb_incorrectly_identified_count, identified_mention_count, identified_verb_count =self.get_model_testing_data(f_gold, f_temp_gold, development_data_dir, model, True, True, margin, dropout_rate)
            # valid_loss = numpy.average(valid_losses)
            # valid_losses = []
            # print('Results on pov data - gold setting: ')                
            # sys.stdout.flush()
            ...
            # print(total_size)
            # sys.stdout.flush()  
            # summary_writer.add_scalar('dev loss', valid_loss, i+1)
            f_gold.close()
            f_temp_gold.close()
            
            #print ('Accuracy of the trained model on CoNLL validation set %f' % (1.0*total_correct_dev / total_size_dev))
            #sys.stdout.flush()
            
            #best_score = total_correct_dev * 1.0 / total_size_dev
            # best_score = total_correct * 1.0 / total_size
...
def main():    
    ...
    parser.add_argument('--conll_data_dir',
                    type=str,
                    default='../generate_training_data/output_training_array_pov_train/conll_padding',
                    help='Directory to the conll training data.')
    parser.add_argument('--development_data_dir',
                    type=str,
                    default='pov_data/test/',
    ...
    parser.add_argument('--pplm',
                    type=int,
                    default=1,
                    help='Plug and play mode.')
