    ...
    def get_updated_variable(self, data_list, max_len):
        ...
        for i in range(len(data_list)):
            ...
            if len(data) < max_len:
                padding = True
                list_x = []
                for j in range(max_len - len(data)):
                    temp = [0] * 768
                    list_x.append(temp)
            ...
            v_4 = None
            if padding:
                v_4 = torch.cat((v_1, v_3, v_x), 0)
            else:
                v_4 = torch.cat((v_1, v_3), 0)
        ...
        for i in range(len(data_list)):
            ...
            for j in range(len(sep_pos)):
                ...
                if sep == 0:
                    ...
                    v_j = Variable(torch.from_numpy(numpy.asarray(list_j, dtype=numpy.float32)), requires_grad=False).type(self.float_type)
            ...
            if last_sep != -1 and last_sep < len(data):
                ...
                v_last = Variable(torch.from_numpy(numpy.asarray(list_last, dtype=numpy.float32)), requires_grad=False).type(self.float_type)
            ...
            if len(sep_pos) == 0:
                v_y = Variable(torch.from_numpy(numpy.asarray(data, dtype=numpy.float32)), requires_grad=False).type(self.float_type)
                if padding:
...
#        ent_len_l1 = Variable(torch.from_numpy(batch_ent_len_l1),
#                          requires_grad=False).type(self.long_type)
#        ent_len_l2 = Variable(torch.from_numpy(batch_ent_len_l2),
#                          requires_grad=False).type(self.long_type)
#        ent_len_r = Variable(torch.from_numpy(batch_ent_len_r),
#                          requires_grad=False).type(self.long_type)
        # Set batch size
...
#        ent_seq_r = torch.cat((ent_seq_r, ent_mention_bit_r, ent_distance_r), dim=2)
        ...
        token_bit_seql_1 = token_bit_seql_1.view(batch_size_l, max_seq_len_l, -1)
        token_bit_seql_2 = token_bit_seql_2.view(batch_size_l, max_seq_len_l, -1)
        token_bit_seqr = token_bit_seqr.view(batch_size_r, max_seq_len_r, -1)
        same_token_bit_seql_1 = same_token_bit_seql_1.view(batch_size_l, max_seq_len_l, -1)
        same_token_bit_seql_2 = same_token_bit_seql_2.view(batch_size_l, max_seq_len_l, -1)
        same_token_bit_seqr = same_token_bit_seqr.view(batch_size_r, max_seq_len_r, -1)
        seq_l1 = torch.cat((seq_l1, token_bit_seql_1, same_token_bit_seql_1, token_distance_seql_1), dim=2)
        seq_l2 = torch.cat((seq_l2, token_bit_seql_2, same_token_bit_seql_2, token_distance_seql_2), dim=2)
        seq_r = torch.cat((seq_r, token_bit_seqr, same_token_bit_seqr, token_distance_seqr), dim=2)
...
#        ent_seq_r = ent_seq_r.transpose(1,0)
        ...
        final_lstm_out_l1 = lstm_out_l1
        
        final_lstm_out_l1 = F.dropout(final_lstm_out_l1, p=self.drop_out_1)

        final_lstm_out_l1 = torch.cat((final_lstm_out_l1, first_mention_bit, second_mention_bit, focus_mention_length, coreference_chain_length, used_bit_seql_1, same_bit_seql_1, is_subject_list, is_object_list), 1)
        
        # Fully connected layers
        fc_out_l1 = F.dropout(torch.tanh(self.linear1(final_lstm_out_l1)), p=self.drop_out_2)

        # Fully connected layer for scoring
        fc_out_l1 = self.linear4(fc_out_l1)

        
        final_lstm_out_l2 = lstm_out_l2
        
        final_lstm_out_l2 = F.dropout(final_lstm_out_l2, p=self.drop_out_1)

        final_lstm_out_l2 = torch.cat((final_lstm_out_l2, first_mention_bit, second_mention_bit, other_mention_length, coreference_chain_length, used_bit_seql_2, same_bit_seql_2, is_subject_list, is_object_list), 1)
        
        # Fully connected layers
        fc_out_l2 = F.dropout(torch.tanh(self.linear1(final_lstm_out_l2)), p=self.drop_out_2)
...
class RNNNet6(object):
    ...
    def get_batch(self, training_data_path, index):
        ...
        token_bit_seq_l1 = data_set[32]
        token_bit_seq_l2 = data_set[33]
        token_bit_seq_r = data_set[34]
        same_token_bit_seq_l1 = data_set[35]
        same_token_bit_seq_l2 = data_set[36]
        same_token_bit_seq_r = data_set[37]
        token_distance_seq_l1 = data_set[38]
        token_distance_seq_l2 = data_set[39]
        token_distance_seq_r = data_set[40]
        
        label = data_set[41]
        max_seql_length = data_set[42]
        max_ent_seql_length = data_set[43]
        max_ent_seqr_length = data_set[44]
        
        return seq_l1, seq_l2, seq_r, len_r, ent_seq_l1, ent_seq_l2, ent_sep_pos_l, ent_seq_r, ent_sep_pos_r, mention_seq_l1, mention_seq_l2, mention_seq_r, distance_seq_l1, distance_seq_l2, distance_seq_r, first_mention_bit, second_mention_bit, focus_mention_length, other_mention_length, coreference_chain_length, used_bit_seql_1, same_bit_seql_1, used_bit_seql_2, same_bit_seql_2, is_subject_list, is_object_list, token_bit_seq_l1, token_bit_seq_l2, token_bit_seq_r, same_token_bit_seq_l1, same_token_bit_seq_l2, same_token_bit_seq_r, token_distance_seq_l1, token_distance_seq_l2, token_distance_seq_r, label, max_seql_length, max_ent_seql_length, max_ent_seqr_length
    ...
    def get_focus_mention_from_line(self, line):
        ...
        if line.startswith("<focus_mention>:"):
            ...
            elif verb_part_2.find("<identified_converted_verb>:") != -1:
        ...
        if is_identified_mention_part == "" and is_annotated_mention_part == "" and is_identified_verb_part == "" and is_annotated_verb_part == "": 
            ...
            if identified_original_focus_mention_part.startswith("<identified_original_focus_mention>:"):
                ...
                original_focus_mention = identified_original_focus_mention
            else:
            ...
            if annotated_original_focus_mention_part.startswith("<annotated_original_focus_mention>:"):
                ...
                for i in range(1,len(temp_parts)):
                    annotated_original_focus_mention = annotated_original_focus_mention + " " + temp_parts[i]
                annotated_original_focus_mention = annotated_original_focus_mention.strip()
                original_focus_mention = annotated_original_focus_mention
            else:
                print(annotated_original_focus_mention_part)
    ...
    def create_padding(self, data_set, max_seql_length, max_seqr_length, max_ent_seql_length, max_ent_seqr_length):
        ...
        token_bit_seql_1v, _ = self.list_to_array(data_set[15], max_seql_length, 1)
        token_bit_seql_2v, _ = self.list_to_array(data_set[16], max_seql_length, 1)
        token_bit_seqr_v, _ = self.list_to_array(data_set[17], max_seqr_length, 1)
        same_token_bit_seql_1v, _ = self.list_to_array(data_set[18], max_seql_length, 1)
        same_token_bit_seql_2v, _ = self.list_to_array(data_set[19], max_seql_length, 1)
        same_token_bit_seql_r, _ = self.list_to_array(data_set[20], max_seqr_length, 1)
        token_distance_bit_seql_1v, _ = self.list_to_array(data_set[21], max_seql_length, 1)
        token_distance_bit_seql_2v, _ = self.list_to_array(data_set[22], max_seql_length, 1)
        token_distance_bit_seqr, _ = self.list_to_array(data_set[23], max_seqr_length, 1)
        return seqr_v, seqr_l, mention_seql_1v, mention_seql_2v, mention_seqr_v, distance_seql_1v, distance_seql_2v, distance_seqr_v, numpy.asarray(data_set[7], dtype=numpy.float32), numpy.asarray(data_set[8], dtype=numpy.float32), numpy.asarray(data_set[9], dtype=numpy.float32), numpy.asarray(data_set[10], dtype=numpy.float32), numpy.asarray(data_set[11], dtype=numpy.float32), numpy.asarray(data_set[12], dtype=numpy.float32), numpy.asarray(data_set[13], dtype=numpy.float32), numpy.asarray(data_set[14], dtype=numpy.float32), token_bit_seql_1v, token_bit_seql_2v, token_bit_seqr_v,same_token_bit_seql_1v, same_token_bit_seql_2v,same_token_bit_seql_r, token_distance_bit_seql_1v, token_distance_bit_seql_2v, token_distance_bit_seqr, numpy.asarray(data_set[24], dtype=numpy.float32)
    ...
    def prune_mention_set_dev(self, mention_set, focus_mention):
        ...
        list2 = ['himself','herself','ourselves','themsevles']
                ...
                for raw_document_data in self.raw_document_data_list:
                    ...
                    for raw_sequence_data in raw_sequence_data_list:    
                        ...
                        if len(mentions_to_replace) == 0:
                            ...
                            updated_related_mentions = []
                            updated_related_mention_cluster_ids = []
                            x = 0
                            ...
                            for sorted_key in temp_dict_sorted_keys:
                                original_related_mention = temp_dict[sorted_key]
                                orms = original_related_mention[0]
                                orme = original_related_mention[1]
                                ...
                                previous_mention_parts = previous_mention.strip().split()
                                correct_previous_mention = correct_previous_mentions[previous_mention_index]
                                correct_previous_mention_parts = correct_previous_mention.strip().split()
                                ...
                                updated_related_mention_cluster_ids.append(previous_mention_cluster_id)
                                if (previous_mention_cluster_id == focus_mention_cluster_id) or (previous_mention_cluster_id in cluster_id_to_he_clusters and focus_mention_cluster_id in cluster_id_to_he_clusters) or (previous_mention_cluster_id in cluster_id_to_she_clusters and focus_mention_cluster_id in cluster_id_to_she_clusters) or (previous_mention_cluster_id in cluster_id_to_it_clusters and focus_mention_cluster_id in cluster_id_to_it_clusters) or (previous_mention_cluster_id in cluster_id_to_they_clusters):
                                    ...
                                    if len(previous_mention_parts) == len(correct_previous_mention_parts):
                                        urms = orms + token_offset
                                        urme = orme + token_offset
                                        updated_related_mention=[urms, urme]
                                        updated_related_mentions.append(updated_related_mention)
                                    else:
                                        urms = orms + token_offset + len(correct_previous_mention_parts) - len(previous_mention_parts)
                                        urme = orme + token_offset
                                        token_offset = token_offset - len(previous_mention_parts) + len(correct_previous_mention_parts)
                                        updated_related_mention=[urms, urme]
                                        updated_related_mentions.append(updated_related_mention)
                            ...
                            for i in range(0,50):                     
                                token_distance_seql_focus[i]=i-previous_m_pos
                                for j in range(0, len(updated_related_mentions)):
                                    related_mention = updated_related_mentions[j]
                                    related_mention_cluster_id = updated_related_mention_cluster_ids[j]
                                    rms = related_mention[0]
                                    rme = related_mention[1]
                                    if i >= rms and i <= rme:
                                        token_bit_seql_focus[i] = 1
                            ...
                            temp_seqr, temp_seqr_text = self.get_embedding_sequence_from_right(sorted_sentence_num_list, sentence_num_to_sentence_embedding_2, all_updated_sentence_num_to_sentence_text_2, related_sentence_num_to_sentence_text, updated_to_token, focus_mention_sentence_num, updated_focus_mention_index_in_sentence, focus_mention_parts, updated_to_sentence)                                
                            seqr.extend(temp_seqr)
                            seqr_text.extend(temp_seqr_text)
                        ...
                        rep_mention_l1 = Variable(torch.from_numpy(batch_mention_seq_l1),
                                requires_grad=False).type(self.float_type)
                        rep_mention_l2 = Variable(torch.from_numpy(batch_mention_seq_l2),
                                requires_grad=False).type(self.float_type)
                        rep_mention_r = Variable(torch.from_numpy(batch_mention_seq_r),
                                requires_grad=False).type(self.float_type)
                        rep_token_bit_l1 = Variable(torch.from_numpy(batch_token_bit_seq_l1),
                                requires_grad=False).type(self.float_type)
                        rep_token_bit_l2 = Variable(torch.from_numpy(batch_token_bit_seq_l2),
                                requires_grad=False).type(self.float_type)
                        rep_token_bit_r = Variable(torch.from_numpy(batch_token_bit_seqr),
                                requires_grad=False).type(self.float_type)
                        rep_same_token_bit_l1 = Variable(torch.from_numpy(batch_same_token_bit_seq_l1),
                                requires_grad=False).type(self.float_type)
                        rep_same_token_bit_l2 = Variable(torch.from_numpy(batch_same_token_bit_seq_l2),
                                requires_grad=False).type(self.float_type)
                        rep_same_token_bit_r = Variable(torch.from_numpy(batch_same_token_bit_seqr),
                                requires_grad=False).type(self.float_type)
                        ...
                        rep_token_distance_l1_array = self.get_distance_embedding(batch_token_distance_seql_1_list)
                        rep_token_distance_l2_array = self.get_distance_embedding(batch_token_distance_seql_2_list)
                        rep_token_distance_r_array = self.get_distance_embedding(batch_token_distance_seqr_list)
                        
                        len_r = Variable(torch.from_numpy(batch_len_r),
                                requires_grad=False).type(self.long_type)               

                        rep_distance_l1 = Variable(torch.from_numpy(rep_distance_l1_array),
                                requires_grad=False).type(self.float_type)
                        rep_distance_l2 = Variable(torch.from_numpy(rep_distance_l2_array),
                                requires_grad=False).type(self.float_type)
                        rep_distance_r = Variable(torch.from_numpy(rep_distance_r_array),
                                requires_grad=False).type(self.float_type)
                        rep_token_distance_l1 = Variable(torch.from_numpy(rep_token_distance_l1_array),
                                requires_grad=False).type(self.float_type)
                        rep_token_distance_l2 = Variable(torch.from_numpy(rep_token_distance_l2_array),
                                requires_grad=False).type(self.float_type)
                        rep_token_distance_r = Variable(torch.from_numpy(rep_token_distance_r_array),
                                requires_grad=False).type(self.float_type)
    ...
    def get_model_testing_data_scenario2(self, f, f_temp, data_dir, model, margin, gold_setting=False, write_result=False):
        # Put model to evaluation mode
        model.eval()
        pov_test_auto_file_list = ['dispatches','faraway','mind','mother','night','nobody','recipe','selkie','understand','walk','water']
                ...
                for raw_document_data in self.raw_document_data_list:
                    ...
                    for raw_sequence_data in raw_sequence_data_list: 
                            ...
                            if gold_setting:
                                ...
                                if is_identified_mention == 1 and is_annotated_mention == 1:
                                    ...
                                    if annotated_original_mention.lower() == identified_original_mention.lower():
                                        ...
                                        if (len(mention_set) > 0) and (mention_set[0].lower() == converted_focus_mention.lower()):
                                    ...
                                    identified_original_mention = raw_sequence_data._identified_original_focus_mention
                                    if (len(mention_set) > 0) and (mention_set[0].lower() != identified_original_mention.lower()):
                        ...
                        if len(mentions_to_replace) == 0:
                            ...
                            seql_focus = []
                            focus_seq_length = 50 + len(converted_focus_mention_parts)
                            ...
                            temp_seqr, temp_seqr_text = self.get_embedding_sequence_from_right(sorted_sentence_num_list, sentence_num_to_sentence_embedding_2,updated_sentence_num_to_sentence_text_2,related_sentence_num_to_sentence_text, end_token_index_in_sentence+end_token_index_offset_2, focus_mention_sentence_num, focus_mention_index_in_sentence, converted_focus_mention_parts)                                
                            seqr.extend(temp_seqr)
                            seqr_text.extend(temp_seqr_text)
                            ...
                            temp_seqr, temp_seqr_text = self.get_embedding_sequence_from_right(sorted_sentence_num_list, sentence_num_to_sentence_embedding_2,all_updated_sentence_num_to_sentence_text_2, related_sentence_num_to_sentence_text, updated_to_token, focus_mention_sentence_num, updated_focus_mention_index_in_sentence, converted_focus_mention_parts, updated_to_sentence)                                
                            seqr.extend(temp_seqr)
                            seqr_text.extend(temp_seqr_text)
                        ...
                        if gold_setting:
                            total_size = total_size + 1
                            current_total_size = current_total_size + 1
                        elif is_identified_mention == 1 and is_annotated_mention == 1:
                            total_annotated_mention = total_annotated_mention + 1
                            current_total_annotated_mention = current_total_annotated_mention + 1
...
#                    
#                    print(data_path)
#                    print("current total annotated mention correctly identified correctly converted: ")
#                    print(current_total_annotated_mention_correctly_identified_correctly_converted)
...
#        conll_gold_testing_data_dir = FLAGS.conll_gold_testing_data_dir
#        pov_gold_testing_data_dir = FLAGS.pov_gold_testing_data_dir
        ...
        output_file = 'result_f_m6_dev_conll_gold_test_three' + ".txt"
        f_output = open(output_file, 'w+', encoding='utf-8')
#        f_temp_gold = open('temp_gold.txt', 'w+', encoding='utf-8')
        f_temp_auto = open('temp_auto.txt', 'w+', encoding='utf-8')
        ...
        for i in range(num_epochs):
            ...
            #while s < 1:
                ...
                rep_token_bit_l2 = Variable(torch.from_numpy(batch_token_bit_seq_l2),
                                requires_grad=False).type(self.float_type)
                rep_token_bit_r = Variable(torch.from_numpy(batch_token_bit_seq_r),
                                requires_grad=False).type(self.float_type)
                rep_same_token_bit_l1 = Variable(torch.from_numpy(batch_same_token_bit_seq_l1),
                                requires_grad=False).type(self.float_type)
                rep_same_token_bit_l2 = Variable(torch.from_numpy(batch_same_token_bit_seq_l2),
                                requires_grad=False).type(self.float_type)
                rep_same_token_bit_r = Variable(torch.from_numpy(batch_same_token_bit_seq_r),
                                requires_grad=False).type(self.float_type)
                rep_distance_l1_array = self.get_distance_embedding(batch_distance_seq_l1)
                rep_distance_l2_array = self.get_distance_embedding(batch_distance_seq_l2)
                rep_distance_r_array = self.get_distance_embedding(batch_distance_seq_r)                
                rep_token_distance_l1_array = self.get_distance_embedding(batch_token_distance_seq_l1)
                rep_token_distance_l2_array = self.get_distance_embedding(batch_token_distance_seq_l2)
                rep_token_distance_r_array = self.get_distance_embedding(batch_token_distance_seq_r)   
...
#            file_name_gold = 'output_f_m6_dev_conll_gold_test_pov_gold' + str(i+1) + ".txt"
#            f_gold = open(file_name_gold, 'w+', encoding='utf-8')
            file_name_auto = 'output_f_m6_dev_conll_gold_test_pov_auto' + str(i+1) + ".txt"
...
#            f_output.write(str(total_correct_dev * 1.0 / total_size_dev))
#            f_output.write("\n")
...
#            valid_losses, total_correct_test_conll_gold, total_size_test_conll_gold = self.get_model_development_data_scenario2(conll_gold_testing_data_dir, model, margin)
#            valid_losses = []
                
#            print ('total testing conll size: ')
#            print(total_size_test_conll_gold)
#            print('total testing conll correct size: ')
#            print(total_correct_test_conll_gold)
#            f_output.write('total testing conll size ')
#            f_output.write('\n')
#            f_output.write(str(total_size_test_conll_gold))
#            f_output.write("\n")
#            f_output.write('total testing conll correct size: ')
#            f_output.write('\n')
#            f_output.write(str(total_correct_test_conll_gold))
#            f_output.write('\n')
...
#            print(total_size)
#            print('\t')
#            print("total annotated mention: ")
#            print(total_annotated_mention)
#            print('\t')
...
#            print('\t')
#            print("total annotated mention incorrectly identified incorrectly converted: ")
#            print(total_annotated_mention_incorrectly_identified_incorrectly_converted)
#            print('\t')
#            print("total annotated mention not identified: ")
#            print(total_annotated_mention_not_identified)
#            print('\t')
#            print("total annotated mention not identified incorrectly converted: ")
...
#            print('\t')
#            print("current total identified mention not annotated incorrectly converted: ")
#            print(total_identified_mention_not_annotated_incorrectly_converted)
#            print('\t')
...
#            print('\t')
#            print("total annotated verb not identified incorrectly converted: ")
#            print(total_annotated_verb_not_identified_incorrectly_converted)
#            print('\t')
#            print("total identified verb not annotated: ")
#            print(total_identified_verb_not_annotated)
#            print('\t')
#            print("total identified verb not annotated incorrectly converted: ")
#            print(total_identified_verb_not_annotated_incorrectly_converted)
#            print('\t')
#            print("total se error: ")
...
#            f_output.write('\t')
#            f_output.write("total annotated mention: ")
#            f_output.write(str(total_annotated_mention))
#            f_output.write('\t')
...
#            f_output.write('\t')
#            f_output.write("total annotated mention incorrectly identified incorrectly converted: ")
#            f_output.write(str(total_annotated_mention_incorrectly_identified_incorrectly_converted))
#            f_output.write('\t')
#            f_output.write("total annotated mention not identified: ")
#            f_output.write(str(total_annotated_mention_not_identified))
#            f_output.write('\t')
#            f_output.write("total annotated mention not identified incorrectly converted: ")
...
#            f_output.write('\t')
#            f_output.write("current total identified mention not annotated incorrectly converted: ")
#            f_output.write(str(total_identified_mention_not_annotated_incorrectly_converted))
#            f_output.write('\t')
...
#            f_output.write('\t')
#            f_output.write("total annotated verb not identified incorrectly converted: ")
#            f_output.write(str(total_annotated_verb_not_identified_incorrectly_converted))
#            f_output.write('\t')
#            f_output.write("total identified verb not annotated: ")
#            f_output.write(str(total_identified_verb_not_annotated))
#            f_output.write('\t')
#            f_output.write("total identified verb not annotated incorrectly converted: ")
#            f_output.write(str(total_identified_verb_not_annotated_incorrectly_converted))
#            f_output.write('\t')
#            f_output.write("total se error: ")
#            f_output.write(str(total_se_error))
#            f_output.write('\n')
            
            valid_losses, total_size, total_correct, total_annotated_mention, total_annotated_mention_correctly_identified, total_annotated_mention_correctly_identified_correctly_converted, total_annotated_mention_incorrectly_identified, total_annotated_mention_incorrectly_identified_incorrectly_converted, total_annotated_mention_not_identified, total_annotated_mention_not_identified_incorrectly_converted, total_identified_mention_not_annotated, total_identified_mention_not_annotated_incorrectly_converted, total_annotated_verb, total_annotated_verb_correctly_identified, total_annotated_verb_correctly_identified_correctly_converted, total_annotated_verb_not_identified, total_annotated_verb_not_identified_incorrectly_converted, total_identified_verb_not_annotated, total_identified_verb_not_annotated_incorrectly_converted, total_se_error=self.get_model_testing_data_scenario2(f_auto, f_temp_auto, pov_auto_testing_data_dir, model, margin, False, True)
            valid_losses = []
            
            correct_mention = total_annotated_mention - (total_annotated_mention_correctly_identified - total_annotated_mention_correctly_identified_correctly_converted) - total_annotated_mention_not_identified_incorrectly_converted - total_annotated_mention_incorrectly_identified_incorrectly_converted
            correct_verb = total_annotated_verb - (total_annotated_verb_correctly_identified - total_annotated_verb_correctly_identified_correctly_converted) - total_annotated_verb_not_identified_incorrectly_converted 
            recall = 1.0 * (correct_mention + correct_verb) / (total_annotated_mention + total_annotated_verb)
            precision = 1.0 * (correct_mention + correct_verb) / (total_annotated_mention_correctly_identified + total_annotated_mention_incorrectly_identified + total_identified_mention_not_annotated_incorrectly_converted + total_annotated_verb_correctly_identified + total_identified_verb_not_annotated_incorrectly_converted)
...
#                    type=str,
#                    default='E:/mention_predictor_upload/code/mention_selection/temp2/conll_padding_scenario_2',
#                    help='Directory to put the conll training data.')
#    parser.add_argument('--development_data_dir',
#                    type=str,
#                    default='E:/mention_predictor_upload/code/mention_selection/output_conll_data/',
#                    help='Directory of the development data.')
#    parser.add_argument('--conll_gold_testing_data_dir',
#                    type=str,
#                    default='E:/mention_predictor_upload/code/mention_selection/output_conll_data/',
#                    help='Path of the testing data.')
#    parser.add_argument('--pov_gold_testing_data_dir',
#                    type=str,
#                    default='E:/mention_predictor_upload/code/mention_selection/output_pov_data/',
#                    help='Path of the testing data.')
...
#    parser.add_argument('--pov_auto_testing_data_dir',
#                    type=str,
#                    default='E:/mention_predictor_upload/code/mention_selection/temp/',
#                    help='Path of the testing data.')
...
#    parser.add_argument('--pov_gold_testing_data_dir',
#                    type=str,
#                    default='/home/mc277509/mention_predictor/output_data/',
#                    help='Path of the testing data.')
    parser.add_argument('--pov_auto_testing_data_dir',
                    type=str,
                    default='pov_data/',
