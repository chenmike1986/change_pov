...
class SuperglueCoNLLProcessor(DataProcessor):
    def __init__(self):
        super().__init__()
        self.labels = [None]
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf8') as f:
            for idx, line in enumerate(f):                
                ...
                meta1['context'] = context
                
                s1=" <M> ".join(example_json['entities'])
                s1 = "<M> " + s1
                meta1['entities'] = s1
                
                # s2="| ".join(example_json['same_entity_mentions'])
                # meta1['same_entity_mentions']=s2
                
                # s3="| ".join(example_json['different_entity_mentions'])
...
PROCESSORS = {
    ...
    "super_glue.conll": SuperglueCoNLLProcessor,
}
