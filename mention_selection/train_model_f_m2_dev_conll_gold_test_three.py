...
class Model_2(NN.Module):
    def __init__(self, use_gpu, lstm_size, hidden_size, drop_out_1, drop_out_2, drop_out_3, drop_out_4):
        ...
        self.random_v = torch.nn.Parameter(torch.randn(1, 768)).cuda()
    ...
    def init_hidden(self, batch_size):
        return (torch.zeros(1, batch_size, self.lstm_size).type(self.float_type).cuda(),
                torch.zeros(1, batch_size, self.lstm_size).type(self.float_type).cuda())
    ...
    def forward(self, seql_1_list, seql_2_list, max_seql_length, rep_r, len_r, ent_seql_1_list, ent_seql_2_list, ent_sep_pos_list_l, ent_seqr_list, ent_sep_pos_list_r, max_ent_seql_length, max_ent_seqr_length, ent_mention_bit_l1, ent_mention_bit_l2, ent_mention_bit_r, ent_distance_l1, ent_distance_l2, ent_distance_r, first_mention_bit, second_mention_bit, focus_mention_length, other_mention_length, coreference_chain_length, used_bit_seql_1, same_bit_seql_1, used_bit_seql_2, same_bit_seql_2, is_subject_list, is_object_list):
        ...
        ent_lstm_out_l1 = self.drop_out_layer3(ent_lstm_out_l1)
        ent_fc_out_l1 = self.drop_out_layer4(torch.tanh(self.linear2(ent_lstm_out_l1)))
        ...
        ent_lstm_out_l2 = self.drop_out_layer3(ent_lstm_out_l2)
        ent_fc_out_l2 = self.drop_out_layer4(torch.tanh(self.linear2(ent_lstm_out_l2)))
        ent_fc_out_l2 = self.linear4(ent_fc_out_l2)
        
        return fc_out_l1+ent_fc_out_l1, fc_out_l2+ent_fc_out_l2
...
class RNNNet2(object):
    ...
    def evaluate(self, model, FLAGS):
        ...
        output_file = 'result_f_m2_dev_conll_gold_test_three_' + ".txt"
        ...
        file_name_gold = 'output_f_m2_dev_conll_gold_test_pov_gold_.txt'
        f_gold = open(file_name_gold, 'w+', encoding='utf-8')
        file_name_test = 'output_f_m2_dev_conll_gold_test_pov_test_.txt'
        ...
        print(print_msg)                   
        valid_losses = []               
        print ('Accuracy of the trained model on CoNLL data %f' % (total_correct_dev / total_size_dev))
    ...
    def train_and_evaluate_scenario2(self, FLAGS):
        ...
        output_file = 'result_f_m2_dev_conll_gold_test_three' + ".txt"
        f_output = open(output_file, 'w+', encoding='utf-8')
        f_temp_gold = open('temp_gold.txt', 'w+', encoding='utf-8')
        f_temp_test = open('temp_test.txt', 'w+', encoding='utf-8')
        ...
        for i in range(num_epochs):
            ...
            start_time = time.time()
            file_name_gold = 'output_f_m2_dev_conll_gold_test_pov_gold' + str(i+1) + ".txt"
            f_gold = open(file_name_gold, 'w+', encoding='utf-8')
            file_name_dev = 'output_f_m2_dev_conll_gold_test_pov_dev' + str(i+1) + ".txt"
            f_dev = open(file_name_dev, 'w+', encoding='utf-8')
            file_name_test = 'output_f_m2_dev_conll_gold_test_pov_test' + str(i+1) + ".txt"
            f_test = open(file_name_test, 'w+', encoding='utf-8')
...
def main():
    ...
    parser.add_argument('--pov_auto_dev_data_dir',
                    type=str,
                    default='pov_data_auto/additional/',
                    help='Path of the testing data.')
