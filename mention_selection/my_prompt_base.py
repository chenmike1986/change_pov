...
class Template(nn.Module):
    ...
    def wrap_one_example(self, 
        ...
        context = context.replace('<F>', '{\"soft\":None, \"soft_id\":1}')
        context = context.replace('</F>', '{\"soft\":None, \"soft_id\":2}')
        context = context.replace('<E>', '{\"soft\":None, \"soft_id\":3}')
        context = context.replace('</E>', '{\"soft\":None, \"soft_id\":4}')
        context = context.replace('@placeholder', '{\"mask\"}')
        entities = example.meta['entities']
        entities = entities.replace('<M>', '{\"soft\":None, \"soft_id\":5}')
        ...
        if isinstance(example, InputExample):
            ...
            for inputflag_name in self.registered_inputflag_names:
                ...
                if hasattr(self, "get_default_"+inputflag_name):
