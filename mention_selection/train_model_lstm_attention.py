# -*- coding: utf-8 -*-
"""
Created on Tue Aug 20 09:57:35 2019
...
class Model_2(NN.Module):
    def __init__(self, use_gpu, lstm_size, hidden_size, drop_out_1, drop_out_2, drop_out_3, drop_out_4):
        ...
        self.random_v = torch.nn.Parameter(torch.randn(1, 768)).to(self.device)
        self.lstm_size = lstm_size
        self.hidden_size = hidden_size
        
        feature_size_1 = 768
        feature_size_2 = feature_size_1 + 7

        self.drop_out_layer1 = torch.nn.Dropout(p=drop_out_1)
        self.drop_out_layer2 = torch.nn.Dropout(p=drop_out_2)
        self.drop_out_layer3 = torch.nn.Dropout(p=drop_out_3)
        self.drop_out_layer4 = torch.nn.Dropout(p=drop_out_4)
              
        self.lstm1 = NN.LSTMCell(feature_size_1, lstm_size)
        self.lstm2 = NN.LSTMCell(feature_size_1, lstm_size)
        self.lstm3 = NN.LSTMCell(feature_size_2, lstm_size)
        self.lstm4 = NN.LSTMCell(feature_size_2, lstm_size)
        
        self.linear1 = NN.Linear(lstm_size * 2 + 18, hidden_size)
        self.linear2 = NN.Linear(lstm_size * 2, hidden_size)       
        self.linear3 = NN.Linear(hidden_size, 1)
        self.linear4 = NN.Linear(hidden_size, 1)

        self.linear5 = NN.Linear(lstm_size*2, hidden_size)
        self.linear6 = NN.Linear(lstm_size*2, hidden_size)
        self.linear7 = NN.Linear(hidden_size, 1)
        self.linear8 = NN.Linear(hidden_size, 1)

        self.linear_tl1 = NN.Linear(lstm_size*2, lstm_size)
        
        self.linear_tl2 = NN.Linear(lstm_size, 1)
        
        self.linear_tr1 = NN.Linear(lstm_size*2, lstm_size)
        
        self.linear_tr2 = NN.Linear(lstm_size, 1)
        
        self.linear_ml1 = NN.Linear(lstm_size*2, lstm_size)
        
        self.linear_ml2 = NN.Linear(lstm_size, 1)
        
        self.linear_mr1 = NN.Linear(lstm_size*2, lstm_size)
        
        self.linear_mr2 = NN.Linear(lstm_size, 1)

        self.drop_out_layer5 = torch.nn.Dropout(0.1)
        self.drop_out_layer6 = torch.nn.Dropout(0.1)
        
    # Initialize the hidden states and cell states of LSTM
    def init_hidden(self, batch_size):
        return (torch.zeros(1, batch_size, self.lstm_size).type(self.float_type).to(self.device),
                torch.zeros(1, batch_size, self.lstm_size).type(self.float_type).to(self.device))
    ...
    def get_updated_variable(self, data_list, max_len):
        ...
        for i in range(len(data_list)):
            data = data_list[i]
            list_1 = data[:50]
            v_1 = Variable(torch.from_numpy(numpy.asarray(list_1, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
            list_3 = data[50:]
            v_3 = Variable(torch.from_numpy(numpy.asarray(list_3, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
            ...
            if len(data) + 1 < max_len:
                ...
                for j in range(max_len - len(data) - 1):
                    temp = [0] * 768
                    list_x.append(temp)
                v_x = Variable(torch.from_numpy(numpy.asarray(list_x, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
            v_4 = None
            if padding:
    ...
    def add_padding(self, data, max_len):
        list_x=data.copy()
        if len(data) + 1 < max_len:
            for j in range(max_len - len(data) - 1):
                temp = [0] * self.lstm_size
                list_x.append(temp)
         
        return list_x
    
    def get_updated_mention_variable(self, data_list, max_len, sep_pos_list):
        result_list = []
        length_list = []
        ...
        for i in range(len(data_list)):
            ...
            for j in range(len(sep_pos)):
                ...
                if sep == 0:
                    temp_result.append(self.random_v)
                else:
                    list_j = data[previous_sep:sep]
                    v_j = Variable(torch.from_numpy(numpy.asarray(list_j, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
                    temp_result.append(v_j)
                    temp_result.append(self.random_v)
                    previous_sep = sep
            if last_sep != -1 and last_sep < len(data):
                list_last = data[last_sep:]
                v_last = Variable(torch.from_numpy(numpy.asarray(list_last, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
                temp_result.append(v_last)
            
            padding = False
            v_x = None
            if len(data) + len(sep_pos) < max_len:
                padding = True
                list_x = []
                for j in range(max_len - len(data) - len(sep_pos)):
                    temp = [0] * 768
                    list_x.append(temp)
                v_x = Variable(torch.from_numpy(numpy.asarray(list_x, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
            if len(sep_pos) == 0:
                v_y = Variable(torch.from_numpy(numpy.asarray(data, dtype=numpy.float32)).to(self.device), requires_grad=False).type(self.float_type)
                if padding:
                    result = torch.cat((v_y, v_x), 0)
                    result_list.append(result)
                else:
                    result_list.append(v_y)
            else:
                if padding:
                    temp_result.append(v_x)
                result = torch.cat(temp_result, 0)
                result_list.append(result)
                
            length = len(data) + len(sep_pos)
    ...
    # Forward process
    def forward(self, seql_1_list, seql_2_list, max_seql_length, rep_r, len_r, ent_seql_1_list, ent_seql_2_list, ent_sep_pos_list_l, ent_seqr_list, ent_sep_pos_list_r, max_ent_seql_length, max_ent_seqr_length, ent_mention_bit_l1, ent_mention_bit_l2, ent_mention_bit_r, ent_distance_l1, ent_distance_l2, ent_distance_r, first_mention_bit, second_mention_bit, focus_mention_length, other_mention_length, coreference_chain_length, used_bit_seql_1, same_bit_seql_1, used_bit_seql_2, same_bit_seql_2, is_subject_list, is_object_list, focus_mention_length_n, other_mention_length_n):
        ...
        len_l1 = torch.tensor(batch_len_l1, dtype=torch.long, requires_grad=False)
        len_l2 = torch.tensor(batch_len_l2, dtype=torch.long, requires_grad=False)
        len_l1 = len_l1.to(self.device)
        len_l2 = len_l2.to(self.device)
        
        ent_l1, batch_ent_len_l1 = self.get_updated_mention_variable(ent_seql_1_list, max_ent_seql_length, ent_sep_pos_list_l)
        ent_l2, batch_ent_len_l2 = self.get_updated_mention_variable(ent_seql_2_list, max_ent_seql_length, ent_sep_pos_list_l)
        ent_r, batch_ent_len_r = self.get_updated_mention_variable(ent_seqr_list, max_ent_seqr_length, ent_sep_pos_list_r)
        
        ent_len_l1 = torch.tensor(batch_ent_len_l1, dtype=torch.long, requires_grad=False)
        ent_len_l2 = torch.tensor(batch_ent_len_l2, dtype=torch.long, requires_grad=False)
        ent_len_r = torch.tensor(batch_ent_len_r, dtype=torch.long, requires_grad=False)
        ent_len_l1 = ent_len_l1.to(self.device)
        ent_len_l2 = ent_len_l2.to(self.device)
        ent_len_r = ent_len_r.to(self.device)
        
        # Set batch size
        batch_size_l = rep_l1.size()[0]
        batch_size_r = rep_r.size()[0]
        ent_batch_size_l = ent_l1.size()[0]
        ent_batch_size_r = ent_r.size()[0]
        max_seq_len_l = rep_l1.size()[1]
        max_seq_len_r = rep_r.size()[1]
        ent_max_seq_len_l = ent_l1.size()[1]
        ent_max_seq_len_r = ent_r.size()[1]
        ...
        ent_seq_l2 = ent_l2
        ent_seq_r = ent_r
        ent_mention_bit_l1 = ent_mention_bit_l1.view(ent_batch_size_l, ent_max_seq_len_l, -1)
        ent_mention_bit_l2 = ent_mention_bit_l2.view(ent_batch_size_l, ent_max_seq_len_l, -1)
        ent_mention_bit_r = ent_mention_bit_r.view(ent_batch_size_r, ent_max_seq_len_r, -1)
        ent_seq_l1 = torch.cat((ent_seq_l1, ent_mention_bit_l1, ent_distance_l1), dim=2)
        ent_seq_l2 = torch.cat((ent_seq_l2, ent_mention_bit_l2, ent_distance_l2), dim=2)
        ent_seq_r = torch.cat((ent_seq_r, ent_mention_bit_r, ent_distance_r), dim=2)
        

        seq_l1 = seq_l1.transpose(1, 0)
        seq_l2 = seq_l2.transpose(1, 0)
        seq_r = seq_r.transpose(1, 0)
        ent_seq_l1 = ent_seq_l1.transpose(1,0)
        ent_seq_l2 = ent_seq_l2.transpose(1,0)
        ent_seq_r = ent_seq_r.transpose(1,0)
        ...
        s_r = (lstm_outs_r.size()[1],)+hidden_two_r.size()
        h_r = hidden_two_r.expand(s_r)
        h_r = h_r.transpose(1,0)              
        t_r = torch.cat((h_r,lstm_outs_r),2)
        attn_weights_r = self.linear_tr2(torch.tanh(self.linear_tr1(t_r)))
        attn_weights_r = attn_weights_r.view(attn_weights_r.size()[0],-1)
        
        attn_probs_r = F.softmax(attn_weights_r, dim=-1)
        #attn_applied_r = torch.matmul(attn_probs_r, lstm_outs_r)
        attn_applied_r = torch.einsum("bl,bli->bi", attn_probs_r, lstm_outs_r)
        
        #attn_applied_r = self.drop_out_layer5(attn_applied_r)
        #hidden_two_r = torch.cat((hidden_two_r,attn_applied_r),1)
        ...
        for i in range(max_seq_len_l):
            ...
            lstm_outs_l1_temp.append(states_l1[0].view(batch_size_l, -1).tolist())
        ...
        lstm_outs_short_l1 = []
        for i in range(lstm_outs_l1.size()[0]):
            len_temp = batch_len_l1[i]-1
            focus_mention_len_temp = int(focus_mention_length_n.tolist()[i])
            temp_index = len_temp - focus_mention_len_temp
            temp_embedding = lstm_outs_l1_temp[:temp_index]            
            temp_embedding = torch.tensor(temp_embedding)
            temp_embedding=temp_embedding[:,i,:]
            temp_embedding = temp_embedding.tolist()
            temp_embedding = self.add_padding(temp_embedding, max_seql_length)
            temp_embedding = torch.tensor(temp_embedding).to(self.device)
            lstm_outs_short_l1.append(temp_embedding.view(1, temp_embedding.size()[0], -1))
        lstm_outs_short_l1 = torch.cat(lstm_outs_short_l1, 0)
        ...
        s_l1 = (lstm_outs_short_l1.size()[1],)+hidden_one_l1.size()
        h_l1 = hidden_one_l1.expand(s_l1)
        h_l1 = h_l1.transpose(1,0)   
        t_l1 = torch.cat((h_l1,lstm_outs_short_l1),2)
        attn_weights_l1 = self.linear_tl2(torch.tanh(self.linear_tl1(t_l1)))
        attn_weights_l1 = attn_weights_l1.view(attn_weights_l1.size()[0],-1)
        
        attn_probs_l1 = F.softmax(attn_weights_l1, dim=-1)
        #attn_applied_l1 = torch.matmul(attn_probs_l1, lstm_outs_short_l1)        
        attn_applied_l1 = torch.einsum("bl,bli->bi", attn_probs_l1, lstm_outs_short_l1)
        
        #attn_applied_l1 = self.drop_out_layer5(attn_applied_l1)
        #hidden_one_l1 = torch.cat((hidden_one_l1,attn_applied_l1),1)
        
        # Concatenate the final hidden state of LSTM_l and LSTM_r
        lstm_out_l1 = torch.cat((hidden_one_l1, hidden_two_r), 1)
        attn_applied_l1 = torch.cat((attn_applied_l1, attn_applied_r), 1)
        attn_applied_l1 = self.drop_out_layer3(attn_applied_l1)
        ...
        for i in range(max_seq_len_l):
            ...
            lstm_outs_l2_temp.append(states_l2[0].view(batch_size_l, -1).tolist())
        ...
        lstm_outs_short_l2 = []
        for i in range(lstm_outs_l2.size()[0]):
            len_temp = batch_len_l2[i]-1
            other_mention_len_temp = int(other_mention_length_n.tolist()[i])
            temp_index = len_temp - other_mention_len_temp
            temp_embedding = lstm_outs_l2_temp[:temp_index]
            temp_embedding = torch.tensor(temp_embedding)
            temp_embedding=temp_embedding[:,i,:]
            temp_embedding = temp_embedding.tolist()
            temp_embedding = self.add_padding(temp_embedding, max_seql_length)
            temp_embedding = torch.tensor(temp_embedding).to(self.device)
            lstm_outs_short_l2.append(temp_embedding.view(1, temp_embedding.size()[0], -1))
        lstm_outs_short_l2 = torch.cat(lstm_outs_short_l2, 0)
        ...
        s_l2 = (lstm_outs_short_l2.size()[1],)+hidden_one_l2.size()
        h_l2 = hidden_one_l2.expand(s_l2)
        h_l2 = h_l2.transpose(1,0)
        t_l2 = torch.cat((h_l2,lstm_outs_short_l2),2)
        attn_weights_l2 = self.linear_tl2(torch.tanh(self.linear_tl1(t_l2)))
        attn_weights_l2 = attn_weights_l2.view(attn_weights_l2.size()[0],-1)
        
        attn_probs_l2 = F.softmax(attn_weights_l2, dim=-1)
        #attn_applied_l2 = torch.matmul(attn_probs_l2, lstm_outs_short_l2)
        attn_applied_l2 = torch.einsum("bl,bli->bi", attn_probs_l2, lstm_outs_short_l2)
        
        #attn_applied_l2 = self.drop_out_layer5(attn_applied_l2)
        #hidden_one_l2 = torch.cat((hidden_one_l2,attn_applied_l2),1)
        
        # Concatenate the final hidden state of LSTM_l and LSTM_r
        lstm_out_l2 = torch.cat((hidden_one_l2, hidden_two_r), 1)
        attn_applied_l2 = torch.cat((attn_applied_l2, attn_applied_r), 1)
        attn_applied_l2 = self.drop_out_layer3(attn_applied_l2)
        
        ent_hidden_r = self.init_hidden(ent_batch_size_r)
        ent_states_r = (ent_hidden_r[0].view(ent_batch_size_r, -1), ent_hidden_r[1].view(ent_batch_size_r, -1))
        
        ent_lstm_outs_r = []
        ent_cell_states_r = []
        for i in range(ent_max_seq_len_r):
            ent_states_r = self.lstm4(ent_seq_r[i], ent_states_r)
            ent_lstm_outs_r.append(ent_states_r[0].view(1, ent_batch_size_r, -1))
            ent_cell_states_r.append(ent_states_r[1].view(1, ent_batch_size_r, -1))
            
        ent_lstm_outs_r = torch.cat(ent_lstm_outs_r, 0)
        
        ent_lstm_outs_r = ent_lstm_outs_r.transpose(1, 0)
        ent_length_r = (ent_len_r-1).view(-1, 1, 1).expand(ent_lstm_outs_r.size(0), 1, ent_lstm_outs_r.size(2))
        ent_hidden_two_r = torch.gather(ent_lstm_outs_r, 1, ent_length_r)
        ent_hidden_two_r = ent_hidden_two_r.transpose(1, 0)
        ent_hidden_two_r = ent_hidden_two_r.view(ent_hidden_two_r.size(1), -1)
        
        ent_s_r = (ent_lstm_outs_r.size()[1],)+ent_hidden_two_r.size()
        ent_h_r = ent_hidden_two_r.expand(ent_s_r)
        ent_h_r = ent_h_r.transpose(1,0)              
        ent_t_r = torch.cat((ent_h_r,ent_lstm_outs_r),2)
        ent_attn_weights_r = self.linear_mr2(torch.tanh(self.linear_mr1(ent_t_r)))
        ent_attn_weights_r = ent_attn_weights_r.view(ent_attn_weights_r.size()[0],-1)
        
        ent_attn_probs_r = F.softmax(ent_attn_weights_r, dim=-1)
        #ent_attn_applied_r = torch.matmul(ent_attn_probs_r, ent_lstm_outs_r)
        ent_attn_applied_r = torch.einsum("bl,bli->bi", ent_attn_probs_r, ent_lstm_outs_r)
        
        #ent_hidden_two_r = torch.cat((ent_hidden_two_r,ent_attn_applied_r),1)
        
        ent_hidden_l1 = self.init_hidden(ent_batch_size_l)
        ent_states_l1 = (ent_hidden_l1[0].view(ent_batch_size_l, -1), ent_hidden_l1[1].view(ent_batch_size_l, -1))

        ent_lstm_outs_l1 = []
        ent_lstm_outs_l1_temp = []
        ent_cell_states_l1 = []
        for i in range(ent_max_seq_len_l):
            ent_states_l1 = self.lstm3(ent_seq_l1[i], ent_states_l1)
            ent_lstm_outs_l1.append(ent_states_l1[0].view(1, ent_batch_size_l, -1))
            ent_lstm_outs_l1_temp.append(ent_states_l1[0].view(ent_batch_size_l, -1).tolist())
            ent_cell_states_l1.append(ent_states_l1[1].view(1, ent_batch_size_l, -1))
    
        ent_lstm_outs_l1 = torch.cat(ent_lstm_outs_l1, 0)

        ent_lstm_outs_l1 = ent_lstm_outs_l1.transpose(1, 0)
        
        ent_lstm_outs_short_l1 = []
        for i in range(ent_lstm_outs_l1.size()[0]):
            len_temp = batch_ent_len_l1[i]-1
            focus_mention_len_temp = int(focus_mention_length_n.tolist()[i])
            temp_index = len_temp - focus_mention_len_temp
            temp_embedding = ent_lstm_outs_l1_temp[:temp_index]
            temp_embedding = torch.tensor(temp_embedding)
            temp_embedding=temp_embedding[:,i,:]
            temp_embedding = temp_embedding.tolist()
            temp_embedding = self.add_padding(temp_embedding, ent_max_seq_len_l)
            temp_embedding = torch.tensor(temp_embedding).to(self.device)
            ent_lstm_outs_short_l1.append(temp_embedding.view(1, temp_embedding.size()[0], -1))
        ent_lstm_outs_short_l1 = torch.cat(ent_lstm_outs_short_l1, 0)
        
        ent_length_l1 = (ent_len_l1-1).view(-1, 1, 1).expand(ent_lstm_outs_l1.size(0), 1, ent_lstm_outs_l1.size(2))
        ent_hidden_one_l1 = torch.gather(ent_lstm_outs_l1, 1, ent_length_l1)
        ent_hidden_one_l1 = ent_hidden_one_l1.transpose(1, 0)
        ent_hidden_one_l1 = ent_hidden_one_l1.view(ent_hidden_one_l1.size(1), -1)

        ent_s_l1 = (ent_lstm_outs_short_l1.size()[1],)+ent_hidden_one_l1.size()
        ent_h_l1 = ent_hidden_one_l1.expand(ent_s_l1)
        ent_h_l1 = ent_h_l1.transpose(1,0)
        ent_t_l1 = torch.cat((ent_h_l1,ent_lstm_outs_short_l1),2)
        ent_attn_weights_l1 = self.linear_ml2(torch.tanh(self.linear_ml1(ent_t_l1)))
        ent_attn_weights_l1 = ent_attn_weights_l1.view(ent_attn_weights_l1.size()[0],-1)
        
        ent_attn_probs_l1 = F.softmax(ent_attn_weights_l1, dim=-1)
        #ent_attn_applied_l1 = torch.matmul(ent_attn_probs_l1, ent_lstm_outs_short_l1)
        ent_attn_applied_l1 = torch.einsum("bl,bli->bi", ent_attn_probs_l1, ent_lstm_outs_short_l1)
        
        #ent_hidden_one_l1 = torch.cat((ent_hidden_one_l1, ent_attn_applied_l1),1)
        
        ent_lstm_out_l1 = torch.cat((ent_hidden_one_l1, ent_hidden_two_r), 1)
        ent_attn_applied_l1 = torch.cat((ent_attn_applied_l1, ent_attn_applied_r), 1)
        ent_attn_applied_l1 = self.drop_out_layer4(ent_attn_applied_l1)
        
        ent_hidden_l2 = self.init_hidden(ent_batch_size_l)
        ent_states_l2 = (ent_hidden_l2[0].view(ent_batch_size_l, -1), ent_hidden_l2[1].view(ent_batch_size_l, -1))

        ent_lstm_outs_l2 = []
        ent_lstm_outs_l2_temp = []
        ent_cell_states_l2 = []
        for i in range(ent_max_seq_len_l):
            ent_states_l2 = self.lstm3(ent_seq_l2[i], ent_states_l2)
            ent_lstm_outs_l2.append(ent_states_l2[0].view(1, ent_batch_size_l, -1))
            ent_lstm_outs_l2_temp.append(ent_states_l2[0].view(ent_batch_size_l, -1).tolist())
            ent_cell_states_l2.append(ent_states_l2[1].view(1, ent_batch_size_l, -1))
    
        ent_lstm_outs_l2 = torch.cat(ent_lstm_outs_l2, 0)

        ent_lstm_outs_l2 = ent_lstm_outs_l2.transpose(1, 0)
        ent_lstm_outs_short_l2 = []
        for i in range(ent_lstm_outs_l2.size()[0]):
            len_temp = batch_ent_len_l2[i]-1
            other_mention_len_temp = int(other_mention_length_n.tolist()[i])
            temp_index = len_temp - other_mention_len_temp
            temp_embedding = ent_lstm_outs_l2_temp[:temp_index]
            temp_embedding = torch.tensor(temp_embedding)
            temp_embedding=temp_embedding[:,i,:]
            temp_embedding = temp_embedding.tolist()
            temp_embedding = self.add_padding(temp_embedding, ent_max_seq_len_l)
            temp_embedding = torch.tensor(temp_embedding).to(self.device)
            ent_lstm_outs_short_l2.append(temp_embedding.view(1, temp_embedding.size()[0], -1))
        ent_lstm_outs_short_l2 = torch.cat(ent_lstm_outs_short_l2, 0)
        
        ent_length_l2 = (ent_len_l2-1).view(-1, 1, 1).expand(ent_lstm_outs_l2.size(0), 1, ent_lstm_outs_l2.size(2))
        ent_hidden_one_l2 = torch.gather(ent_lstm_outs_l2, 1, ent_length_l2)
        ent_hidden_one_l2 = ent_hidden_one_l2.transpose(1, 0)
        ent_hidden_one_l2 = ent_hidden_one_l2.view(ent_hidden_one_l2.size(1), -1)        
        
        ent_s_l2 = (ent_lstm_outs_short_l2.size()[1],)+ent_hidden_one_l2.size()
        ent_h_l2 = ent_hidden_one_l2.expand(ent_s_l2)
        ent_h_l2 = ent_h_l2.transpose(1,0)
        ent_t_l2 = torch.cat((ent_h_l2,ent_lstm_outs_short_l2),2)
        ent_attn_weights_l2 = self.linear_ml2(torch.tanh(self.linear_ml1(ent_t_l2)))
        ent_attn_weights_l2 = ent_attn_weights_l2.view(ent_attn_weights_l2.size()[0],-1)
        
        ent_attn_probs_l2 = F.softmax(ent_attn_weights_l2, dim=-1)
        #ent_attn_applied_l2 = torch.matmul(ent_attn_probs_l2, ent_lstm_outs_short_l2)
        ent_attn_applied_l2 = torch.einsum("bl,bli->bi", ent_attn_probs_l2, ent_lstm_outs_short_l2)
        
        #ent_hidden_one_l2 = torch.cat((ent_hidden_one_l2, ent_attn_applied_l2),1)
        
        ent_lstm_out_l2 = torch.cat((ent_hidden_one_l2, ent_hidden_two_r), 1)
        ent_attn_applied_l2 = torch.cat((ent_attn_applied_l2, ent_attn_applied_r), 1)
        ent_attn_applied_l2 = self.drop_out_layer4(ent_attn_applied_l2)
        
        lstm_out_l1 = self.drop_out_layer1(lstm_out_l1)
        ...
        fc_out_l1 = self.drop_out_layer2(torch.tanh(self.linear1(lstm_out_l1)))
        fc_out_l1 = self.linear3(fc_out_l1)

        attn_score_l1 = torch.tanh(self.linear5(attn_applied_l1))
        attn_score_l1 = self.linear7(attn_score_l1)
        
        ent_lstm_out_l1 = self.drop_out_layer5(ent_lstm_out_l1)
        ent_fc_out_l1 = self.drop_out_layer6(torch.tanh(self.linear2(ent_lstm_out_l1)))
        ent_fc_out_l1 = self.linear4(ent_fc_out_l1)   
        
        ent_attn_score_l1 = torch.tanh(self.linear6(ent_attn_applied_l1))
        ent_attn_score_l1 = self.linear8(ent_attn_score_l1)
        
        lstm_out_l2 = self.drop_out_layer1(lstm_out_l2)
        ...
        fc_out_l2 = self.drop_out_layer2(torch.tanh(self.linear1(lstm_out_l2)))
        fc_out_l2 = self.linear3(fc_out_l2)    
        
        attn_score_l2 = torch.tanh(self.linear5(attn_applied_l2))
        attn_score_l2 = self.linear7(attn_score_l2)
        
        ent_lstm_out_l2 = self.drop_out_layer5(ent_lstm_out_l2)
        ent_fc_out_l2 = self.drop_out_layer6(torch.tanh(self.linear2(ent_lstm_out_l2)))
        ent_fc_out_l2 = self.linear4(ent_fc_out_l2)
        
        ent_attn_score_l2 = torch.tanh(self.linear6(ent_attn_applied_l2))
        ent_attn_score_l2 = self.linear8(ent_attn_score_l2)
        
        return fc_out_l1+attn_score_l1+ent_fc_out_l1+ent_attn_score_l1, fc_out_l2+attn_score_l2+ent_fc_out_l2+ent_attn_score_l2
        
###############################################################
# Recurrent neural network class
#
class RNNNet2(object):
    ...
    def get_bert_embedding(self, text):
        ...
        result = self.bert_model(tokens_ids_tensor, token_type_ids=segments_tensors, attention_mask=input_mask)
        
        last_hidden_state = result.last_hidden_state
        required_encoder_layer = last_hidden_state.view(-1, 768)
    ...
    def get_batch(self, training_data_path, index):
        ...
        seq_r = data_set[2]
        len_r = data_set[3]
        ent_seq_l1 = data_set[4]
        ent_seq_l2 = data_set[5]
        ent_sep_pos_l = data_set[6]
        ent_seq_r = data_set[7]
        ent_sep_pos_r = data_set[8]
        mention_seq_l1 = data_set[9]
        mention_seq_l2 = data_set[11]
        mention_seq_r = data_set[13]
        distance_seq_l1 = data_set[15]
        distance_seq_l2 = data_set[17]
        distance_seq_r = data_set[19]
        ...
        max_ent_seql_length = data_set[34]
        max_ent_seqr_length = data_set[35]
        
        return seq_l1, seq_l2, seq_r, len_r, ent_seq_l1, ent_seq_l2, ent_sep_pos_l, ent_seq_r, ent_sep_pos_r, mention_seq_l1, mention_seq_l2, mention_seq_r, distance_seq_l1, distance_seq_l2, distance_seq_r, first_mention_bit, second_mention_bit, focus_mention_length, other_mention_length, coreference_chain_length, used_bit_seql_1, same_bit_seql_1, used_bit_seql_2, same_bit_seql_2, is_subject_list, is_object_list, label, max_seql_length, max_ent_seql_length, max_ent_seqr_length
    ...
    def create_padding(self, data_set, max_seqr_length, max_ent_seql_length, max_ent_seqr_length):
        seqr_v, seqr_l = self.list_to_array(data_set[0], max_seqr_length, 2)
        mention_seql_1v, mention_seql_1l = self.list_to_array(data_set[1], max_ent_seql_length, 1)
        mention_seql_2v, mention_seql_2l = self.list_to_array(data_set[2], max_ent_seql_length, 1)
        mention_seqr_v, mention_seqr_l = self.list_to_array(data_set[3], max_ent_seqr_length, 1)
        distance_seql_1v, distance_seql_1l = self.list_to_array(data_set[4], max_ent_seql_length, 1)
        distance_seql_2v, distance_seql_2l = self.list_to_array(data_set[5], max_ent_seql_length, 1)
        distance_seqr_v, distance_seqr_l = self.list_to_array(data_set[6], max_ent_seqr_length, 1)
        return seqr_v, seqr_l, mention_seql_1v, mention_seql_2v, mention_seqr_v, distance_seql_1v, distance_seql_2v, distance_seqr_v, numpy.asarray(data_set[7], dtype=numpy.float32), numpy.asarray(data_set[8], dtype=numpy.float32), numpy.asarray(data_set[9], dtype=numpy.float32), numpy.asarray(data_set[10], dtype=numpy.float32), numpy.asarray(data_set[11], dtype=numpy.float32), numpy.asarray(data_set[12], dtype=numpy.float32), numpy.asarray(data_set[13], dtype=numpy.float32), numpy.asarray(data_set[14], dtype=numpy.float32), numpy.asarray(data_set[15], dtype=numpy.float32)
    ...
    def get_distance_embedding(self, batch_distance_seq):
        rep_distance_list = []
        for x in range(batch_distance_seq.shape[0]):
            temp_set = []
            for y in range(batch_distance_seq.shape[1]):
                z = batch_distance_seq[x][y]
                if z <= 5:
                    temp_list = [1, 0, 0, 0, 0, 0]
                elif z > 5 and z <= 10:
                    temp_list = [0, 1, 0, 0, 0, 0]
                elif z > 10 and z <= 15:
                    temp_list = [0, 0, 1, 0, 0, 0]
                elif z > 15 and z <= 20:
                    temp_list = [0, 0, 0, 1, 0, 0]
                elif z > 20 and z <= 25:
                    temp_list = [0, 0, 0, 0, 1, 0]
                else:
                    temp_list = [0, 0, 0, 0, 0, 1]
                        
                temp_set.append(temp_list)
            rep_distance_list.append(temp_set)
    ...
    def get_model_development_data_scenario2(self, data_dir, model, margin):
        ...
        with torch.no_grad():
            ...
            for data_path in files:
                ...
                previous_entities = []
                previous_entity_cluster_ids = []
                correct_previous_entities = []
                previous_entity_index_in_sentence_list = []
                previous_entity_sentence_num_list = []
                previous_entity_sentence_text_list = []
                ...
                for raw_document_data in self.raw_document_data_list:
                    ...
                    for raw_sequence_data in raw_sequence_data_list:    
                        ...
                        valid_pre_mention_size = 0
                        for pre_mention_cluster_id in pre_mention_cluster_id_sequence:
                            if pre_mention_cluster_id in invalid_cluster_id_to_cluster_data or pre_mention_cluster_id == -1:
                                continue
                            else:
                                valid_pre_mention_size = valid_pre_mention_size + 1
                        ...
                        if len(mention_set) < 2:
                            ...
                            previous_entities.append(focus_mention)
                            previous_entity_cluster_ids.append(focus_mention_cluster_id)
                            correct_previous_entities.append(focus_mention)
                            previous_entity_index_in_sentence_list.append(focus_mention_index_in_sentence)
                            previous_entity_sentence_num_list.append(focus_mention_sentence_num)
                            previous_entity_sentence_text_list.append(related_sentence_num_to_sentence_text[focus_mention_sentence_num])
                        ...
                        max_seql_length = 0
                        max_seqr_length = 0
                        max_ent_seql_length = 0
                        max_ent_seqr_length = 0
                        ...
                        ent_sep_pos_l = []
                        ent_sep_pos_count_l = 0
                    
                        invalid_pre_mention_count = 0
                        for i in range(len(pre_mention_sequence)-1):
                            ...
                            if ms == '<pad>':
                                ...
                                invalid_pre_mention_count = invalid_pre_mention_count + 1
                                ent_seql_focus.extend(token_embeddings_for_mention)
                                continue
                            elif ms_cluster_id in invalid_cluster_id_to_cluster_data:
                                invalid_pre_mention_count = invalid_pre_mention_count + 1
                            elif (ms_cluster_id == focus_mention_cluster_id) or (ms_cluster_id in cluster_id_to_he_clusters and focus_mention_cluster_id in cluster_id_to_he_clusters) or (ms_cluster_id in cluster_id_to_she_clusters and focus_mention_cluster_id in cluster_id_to_she_clusters) or (ms_cluster_id in cluster_id_to_it_clusters and focus_mention_cluster_id in cluster_id_to_it_clusters) or (ms_cluster_id in cluster_id_to_they_clusters):
                                ms = previous_entities[len(previous_entities)- valid_pre_mention_size + i + 1 - invalid_pre_mention_count]

                            ent_sep_pos_l.append(len(ent_seql_focus))
                            ent_sep_pos_count_l = ent_sep_pos_count_l + 1
                            ...
                            updated_pre_mention_sentence_text = self.get_updated_sentence(pre_mention_sentence_text, pre_mention_index_in_sentence, ms, len(pre_mention_text_parts))
                        
                            token_embeddings_for_sentence = self.get_bert_embedding(updated_pre_mention_sentence_text)
                            ms_parts = ms.split()
                            for j in range(pre_mention_index_in_sentence, pre_mention_index_in_sentence + len(ms_parts)):
                        ...
                        focus_mention_sentence_text = related_sentence_num_to_sentence_text[focus_mention_sentence_num]
                        token_embeddings_for_sentence = None
                        if focus_mention_sentence_num not in original_sentence_num_to_embedding:
                            token_embeddings_for_sentence = self.get_bert_embedding(focus_mention_sentence_text)
                            original_sentence_num_to_embedding[focus_mention_sentence_num] = token_embeddings_for_sentence
                        else:
                            token_embeddings_for_sentence = original_sentence_num_to_embedding[focus_mention_sentence_num]
                    
                        ent_sep_pos_l.append(len(ent_seql_focus))
                        ent_sep_pos_count_l = ent_sep_pos_count_l + 1
                        mention_bit_seql_focus.append(1)
                        mention_distance_seql_focus.append(pre_mention_distance_sequence[len(pre_mention_distance_sequence)-1])
                        
                        for i in range(focus_mention_index_in_sentence, focus_mention_index_in_sentence + len(focus_mention_parts)):
                            ent_seql_focus.append(token_embeddings_for_sentence[i])
                            mention_bit_seql_focus.append(1)
                            mention_distance_seql_focus.append(pre_mention_distance_sequence[len(pre_mention_distance_sequence)-1])
                    
                        ent_focus_seq_length = len(ent_seql_focus) + len(ent_sep_pos_l)
                        if max_ent_seql_length < ent_focus_seq_length:
                            max_ent_seql_length = ent_focus_seq_length
                        ...
                        for i in range(len(post_mention_sequence)-1, -1, -1):
                            ...
                            if ms == '<pad>':
                                ...
                                ent_sep_pos_count_r = ent_sep_pos_count_r + 1
                                if ms_cluster_id == focus_mention_cluster_id:
                                ...
                                if (ms_cluster_id == focus_mention_cluster_id) or (ms_cluster_id in cluster_id_to_he_clusters and focus_mention_cluster_id in cluster_id_to_he_clusters) or (ms_cluster_id in cluster_id_to_she_clusters and focus_mention_cluster_id in cluster_id_to_she_clusters) or (ms_cluster_id in cluster_id_to_it_clusters and focus_mention_cluster_id in cluster_id_to_it_clusters) or (ms_cluster_id in cluster_id_to_they_clusters):
                        ...
                        if max_ent_seqr_length < ent_seqr_length:
                            max_ent_seqr_length = ent_seqr_length
                        ...
                        for i in range(len(mention_set)):
                            ...
                            if mention.lower() != focus_mention.lower():
                                ...
                                updated_sentence_text = self.get_updated_sentence(focus_mention_sentence_text, focus_mention_index_in_sentence, mention, len(focus_mention_parts))
                                ...
                                if max_ent_seql_length < ent_other_seq_length:
                                    max_ent_seql_length = ent_other_seq_length
                                ...
                                ent_seql_focus_copy_1 = ent_seql_focus.copy()
                                ent_seql_focus_copy_2 = ent_seql_focus.copy()
                                mention_bit_seql_focus_copy_1 = mention_bit_seql_focus.copy()
                                mention_bit_seql_focus_copy_2 = mention_bit_seql_focus.copy()
                                mention_distance_seql_focus_copy_1 = mention_distance_seql_focus.copy()
                                mention_distance_seql_focus_copy_2 = mention_distance_seql_focus.copy()
                                ent_seqr_copy = ent_seqr.copy()
                                mention_bit_seqr_copy = mention_bit_seqr.copy()
                                mention_distance_seqr_copy = mention_distance_seqr.copy()
                                ent_sep_pos_l_copy = ent_sep_pos_l.copy()
                                ent_sep_pos_r_copy = ent_sep_pos_r.copy()
                                ent_seql_1_list.append(ent_seql_focus_copy_1)
                                ent_seql_2_list.append(ent_seql_focus_copy_2)
                                ent_seqr_list.append(ent_seqr_copy)
                                mention_bit_seql_1_list.append(mention_bit_seql_focus_copy_1)
                                mention_bit_seql_2_list.append(mention_bit_seql_focus_copy_2)
                                mention_bit_seqr_list.append(mention_bit_seqr_copy)
                                mention_distance_seql_1_list.append(mention_distance_seql_focus_copy_1)
                                mention_distance_seql_2_list.append(mention_distance_seql_focus_copy_2)
                                mention_distance_seqr_list.append(mention_distance_seqr_copy)
                                ent_sep_pos_list_l.append(ent_sep_pos_l_copy)
                                ent_sep_pos_list_r.append(ent_sep_pos_r_copy)
                        pre_padding_count = 0
                        ...
                        if len(mentions_to_replace) == 0:
                            ...
                            for i in range(len(mention_set)):
                                ...
                                if mention.lower() != focus_mention.lower():                                
                                    ...
                                    seql_focus_copy_1 = seql_focus.copy()
                                    seql_focus_copy_2 = seql_focus.copy()
                                    seqr_copy = seqr.copy()
                                    seql_1_list.append(seql_focus_copy_1)
                                    seql_2_list.append(seql_focus_copy_2)
                                    seqr_list.append(seqr_copy)
                        ...
                        total_size = total_size + 1
                        batch_r, batch_len_r, batch_mention_seq_l1, batch_mention_seq_l2, batch_mention_seq_r, batch_distance_seq_l1, batch_distance_seq_l2, batch_distance_seq_r, batch_first_mention_bit, batch_second_mention_bit, batch_used_bit_seql_1, batch_same_bit_seql_1, batch_used_bit_seql_2, batch_same_bit_seql_2, batch_is_subject_list, batch_is_object_list, batch_label = \
                        self.create_padding([seqr_list, mention_bit_seql_1_list, mention_bit_seql_2_list, mention_bit_seqr_list, mention_distance_seql_1_list, mention_distance_seql_2_list, mention_distance_seqr_list, first_mention_bit_seql_list, second_mention_bit_seql_list, used_bit_seql_1_list, same_bit_seql_1_list, used_bit_seql_2_list, same_bit_seql_2_list, is_subject_list, is_object_list, label_list], max_seqr_length, max_ent_seql_length, max_ent_seqr_length)
                        ...
                        rep_r = torch.tensor(batch_r, dtype=torch.float32, requires_grad=False)
                        rep_r = rep_r.to(self.device)
                        rep_mention_l1 = torch.tensor(batch_mention_seq_l1, dtype=torch.float32, requires_grad=False)
                        rep_mention_l2 = torch.tensor(batch_mention_seq_l2, dtype=torch.float32, requires_grad=False)
                        rep_mention_r = torch.tensor(batch_mention_seq_r, dtype=torch.float32, requires_grad=False)
                        rep_mention_l1 = rep_mention_l1.to(self.device)
                        rep_mention_l2 = rep_mention_l2.to(self.device)
                        rep_mention_r = rep_mention_r.to(self.device)
                        
                        rep_distance_l1_array = self.get_distance_embedding(batch_distance_seq_l1)
                        rep_distance_l2_array = self.get_distance_embedding(batch_distance_seq_l2)
                        rep_distance_r_array = self.get_distance_embedding(batch_distance_seq_r) 
 
                        len_r = torch.tensor(batch_len_r, dtype=torch.long, requires_grad=False)
                        len_r = len_r.to(self.device)
                        
                        rep_distance_l1 = torch.tensor(rep_distance_l1_array, dtype=torch.float32, requires_grad=False)
                        rep_distance_l2 = torch.tensor(rep_distance_l2_array, dtype=torch.float32, requires_grad=False)
                        rep_distance_r = torch.tensor(rep_distance_r_array, dtype=torch.float32, requires_grad=False)
                        rep_distance_l1 = rep_distance_l1.to(self.device)
                        rep_distance_l2 = rep_distance_l2.to(self.device)
                        rep_distance_r = rep_distance_r.to(self.device)
                        
                        rep_first_mention_bit = torch.tensor(batch_first_mention_bit, dtype=torch.float32, requires_grad=False)
                        rep_second_mention_bit = torch.tensor(batch_second_mention_bit, dtype=torch.float32, requires_grad=False)
                        rep_first_mention_bit = rep_first_mention_bit.to(self.device)
                        rep_second_mention_bit = rep_second_mention_bit.to(self.device)
                        
                        rep_focus_mention_length = torch.tensor(rep_focus_mention_length_array, dtype=torch.float32, requires_grad=False)
                        rep_other_mention_length = torch.tensor(rep_other_mention_length_array, dtype=torch.float32, requires_grad=False)
                        rep_coreference_chain_length = torch.tensor(rep_coreference_chain_length_array, dtype=torch.float32, requires_grad=False)
                        
                        rep_focus_mention_length = rep_focus_mention_length.to(self.device)
                        rep_other_mention_length = rep_other_mention_length.to(self.device)
                        rep_coreference_chain_length = rep_coreference_chain_length.to(self.device)
                        
                        focus_mention_length_n = torch.tensor(focus_mention_length_list, dtype=torch.float32, requires_grad=False).to(self.device)
                        other_mention_length_n = torch.tensor(other_mention_length_list, dtype=torch.float32, requires_grad=False).to(self.device)
                        
                        rep_used_bit_seql_1 = torch.tensor(batch_used_bit_seql_1, dtype=torch.float32, requires_grad=False)
                        rep_same_bit_seql_1 = torch.tensor(batch_same_bit_seql_1, dtype=torch.float32, requires_grad=False)
                        rep_used_bit_seql_2 = torch.tensor(batch_used_bit_seql_2, dtype=torch.float32, requires_grad=False)
                        rep_same_bit_seql_2 = torch.tensor(batch_same_bit_seql_2, dtype=torch.float32, requires_grad=False)
                        rep_used_bit_seql_1 = rep_used_bit_seql_1.to(self.device)
                        rep_same_bit_seql_1 = rep_same_bit_seql_1.to(self.device)
                        rep_used_bit_seql_2 = rep_used_bit_seql_2.to(self.device)
                        rep_same_bit_seql_2 = rep_same_bit_seql_2.to(self.device)
                        
                        rep_is_subject_list = torch.tensor(batch_is_subject_list, dtype=torch.float32, requires_grad=False)
                        rep_is_object_list = torch.tensor(batch_is_object_list, dtype=torch.float32, requires_grad=False)
                        label = torch.tensor(batch_label, dtype=torch.float32, requires_grad=False)

                        rep_is_subject_list = rep_is_subject_list.to(self.device)
                        rep_is_object_list = rep_is_object_list.to(self.device)
                        label = label.to(self.device)
                        
                        # Forward pass: predict scores
                        score_l1, score_l2 = model(seql_1_list, seql_2_list, max_seql_length, rep_r, len_r, ent_seql_1_list, ent_seql_2_list, ent_sep_pos_list_l, ent_seqr_list, ent_sep_pos_list_r, max_ent_seql_length, max_ent_seqr_length, rep_mention_l1, rep_mention_l2, rep_mention_r, rep_distance_l1, rep_distance_l2, rep_distance_r, rep_first_mention_bit, rep_second_mention_bit, rep_focus_mention_length, rep_other_mention_length, rep_coreference_chain_length, rep_used_bit_seql_1, rep_same_bit_seql_1, rep_used_bit_seql_2, rep_same_bit_seql_2, rep_is_subject_list, rep_is_object_list, focus_mention_length_n, other_mention_length_n)
                        ...
                        for ii in range(len(mention_set)):
                            ...
                            if ii == max_index:                                    
                                ...
                                previous_mention_index_in_sentence_list.append(focus_mention_index_in_sentence)
                                previous_mention_sentence_num_list.append(focus_mention_sentence_num)
                                previous_entities.append(mention)
                                previous_entity_cluster_ids.append(focus_mention_cluster_id)
                                correct_previous_entities.append(focus_mention)
                                previous_entity_index_in_sentence_list.append(focus_mention_index_in_sentence)
                                previous_entity_sentence_num_list.append(focus_mention_sentence_num)
            ...
            for data_path in files:
                ...
                for raw_document_data in self.raw_document_data_list:
                    ...
                    for raw_sequence_data in raw_sequence_data_list: 
                        ...
                        start_token_index_in_sentence = raw_sequence_data._start_token_index_in_sentence
                        end_token_index_in_sentence = raw_sequence_data._end_token_index_in_sentence
                        original_pre_mention_sequence = raw_sequence_data._original_pre_mention_sequence
                        converted_pre_mention_sequence = raw_sequence_data._converted_pre_mention_sequence
                        pre_mention_info_list = raw_sequence_data._pre_mention_info_list
                        pre_mention_cluster_id_sequence = raw_sequence_data._pre_mention_cluster_id_sequence
                        pre_mention_distance_sequence = raw_sequence_data._pre_mention_distance_sequence
                        original_post_mention_sequence= raw_sequence_data._original_post_mention_sequence
                        converted_post_mention_sequence = raw_sequence_data._converted_post_mention_sequence
                        post_mention_info_list = raw_sequence_data._post_mention_info_list
                        post_mention_cluster_id_sequence = raw_sequence_data._post_mention_cluster_id_sequence
                        post_mention_distance_sequence = raw_sequence_data._post_mention_distance_sequence
                        ...
                        valid_pre_mention_size = 0
                        for pre_mention_cluster_id in pre_mention_cluster_id_sequence:
                            if pre_mention_cluster_id in gold_invalid_cluster_id_to_cluster_data or pre_mention_cluster_id == -3:
                                continue
                            else:
                                valid_pre_mention_size = valid_pre_mention_size + 1
                        ...
                        if len(mention_set) < 2:
                            ...
                            previous_mention_index_in_sentence_list.append(focus_mention_index_in_sentence)
                            previous_mention_sentence_num_list.append(focus_mention_sentence_num)
                            previous_entities.append(converted_focus_mention)
                            previous_entity_cluster_ids.append(focus_mention_cluster_id)
                            correct_previous_entities.append(original_focus_mention)
                            previous_entity_index_in_sentence_list.append(focus_mention_index_in_sentence)
                            previous_entity_sentence_num_list.append(focus_mention_sentence_num)
                            previous_entity_sentence_text_list.append(related_sentence_num_to_sentence_text[focus_mention_sentence_num])
                        ...
                        ent_sep_pos_count_l = 0
                        invalid_pre_mention_count = 0
                        for i in range(len(converted_pre_mention_sequence)-1):
                            ms = converted_pre_mention_sequence[i]
                            ms_cluster_id = pre_mention_cluster_id_sequence[i]
                            ms_distance = pre_mention_distance_sequence[i]
                            token_embeddings_for_mention = []                               
                            if ms == '<pad>':
                                ...
                                ent_seql_focus.extend(token_embeddings_for_mention)
                                continue
                            elif ms_cluster_id in gold_invalid_cluster_id_to_cluster_data:
                                invalid_pre_mention_count = invalid_pre_mention_count + 1
                            elif (ms_cluster_id == focus_mention_cluster_id) or (ms_cluster_id in cluster_id_to_he_clusters and focus_mention_cluster_id in cluster_id_to_he_clusters) or (ms_cluster_id in cluster_id_to_she_clusters and focus_mention_cluster_id in cluster_id_to_she_clusters) or (ms_cluster_id in cluster_id_to_it_clusters and focus_mention_cluster_id in cluster_id_to_it_clusters) or (ms_cluster_id in cluster_id_to_they_clusters):
                                ms = previous_entities[len(previous_entities)- valid_pre_mention_size + i + 1 - invalid_pre_mention_count]
                        ...
                        last_pre_mention_info = pre_mention_info_list[len(pre_mention_info_list)-1]
                        last_pre_mention_text = last_pre_mention_info._text
                        last_pre_mention_text_parts = last_pre_mention_text.split()
                        last_pre_mention_index_in_sentence = last_pre_mention_info._index_in_sentence
                        last_pre_mention_sentence_text = last_pre_mention_info._sentence_text
                    
                        token_embeddings_for_sentence = self.get_bert_embedding(last_pre_mention_sentence_text)
                        ...
                        for i in range(last_pre_mention_index_in_sentence, last_pre_mention_index_in_sentence + len(last_pre_mention_text_parts)):
                            ent_seql_focus.append(token_embeddings_for_sentence[i])
                            mention_bit_seql_focus.append(1)
                            mention_distance_seql_focus.append(pre_mention_distance_sequence[len(pre_mention_distance_sequence)-1])
                        ...
                        ent_sep_pos_r = []
                        ent_sep_pos_count_r = 0
                        for i in range(len(converted_post_mention_sequence)-1, -1, -1):
                            ms = converted_post_mention_sequence[i]
                            ms_cluster_id = post_mention_cluster_id_sequence[i]
                            ms_distance = post_mention_distance_sequence[i]
                            ms_parts = ms.split()
                            token_embeddings_for_mention = []
                        ...
                        for i in range(len(mention_set)):
                            ...
                            if mention.lower() != converted_focus_mention.lower():
                                ...
                                for j in range(len(ent_seql_focus) - len(last_pre_mention_text_parts)):
                                    ent_seql_other.append(ent_seql_focus[j])
                                
                                for j in range(len(mention_bit_seql_focus) - len(last_pre_mention_text_parts)):
                                    mention_bit_seql_other.append(mention_bit_seql_focus[j])
                                    mention_distance_seql_other.append(mention_distance_seql_focus[j])

                                updated_sentence_text = self.get_updated_sentence(last_pre_mention_sentence_text, last_pre_mention_index_in_sentence, mention, len(last_pre_mention_text_parts))
                                token_embeddings_for_sentence = self.get_bert_embedding(updated_sentence_text)
                                for j in range(last_pre_mention_index_in_sentence, last_pre_mention_index_in_sentence + len(mention_parts)):
                                    ent_seql_other.append(token_embeddings_for_sentence[j])
                                    mention_bit_seql_other.append(1)
                                    mention_distance_seql_other.append(pre_mention_distance_sequence[len(pre_mention_distance_sequence)-1])
                                token_embeddings_for_sentence = []
                        ...
                        for ii in range(len(mention_set)):
                            ...
                            if ii == max_index:
                                ...
                                previous_mention_sentence_num_list.append(focus_mention_sentence_num)
                                chosen_mention = mention
                                previous_entities.append(chosen_mention)
                                previous_entity_cluster_ids.append(focus_mention_cluster_id)
                                correct_previous_entities.append(original_focus_mention)
                                previous_entity_index_in_sentence_list.append(focus_mention_index_in_sentence)
                                previous_entity_sentence_num_list.append(focus_mention_sentence_num)
    ...
    def evaluate(self, model, FLAGS):
        ...
        output_file = 'result_lstm_attention' + ".txt"
        f_output = open(output_file, 'w+', encoding='utf-8')
        f_temp_gold = open('temp_gold_lstm_attention.txt', 'w+', encoding='utf-8')
        f_temp_test = open('temp_test_lstm_attention.txt', 'w+', encoding='utf-8')
        file_name_gold = 'output_lstm_attention_gold.txt'
        f_gold = open(file_name_gold, 'w+', encoding='utf-8')
        file_name_test = 'output_lstm_attention_auto.txt'
        ...
        print(print_msg)                   
        valid_losses = []               
        print ('Accuracy of the trained model on CoNLL test data %f' % (1.0*total_correct_dev / total_size_dev))
        ...
        print("accuracy: ")
        print(total_correct * 1.0 / total_size)
    ...
    def train_and_evaluate_scenario2(self, FLAGS):
        ...
        drop_rate_3 = FLAGS.dropout_rate_3
        drop_rate_4 = FLAGS.dropout_rate_4
        ...
        model = eval("Model_" + str(self.mode))(
                    self.use_gpu, lstm_size, hidden_size, drop_rate_1, drop_rate_2, drop_rate_3, drop_rate_4)
    
        #pretrained_dict = torch.load(loaded_model_path)
        ...
        for i in range(num_epochs):
            ...
            #while s < 1:
                ...
                batch_l1, batch_l2, batch_r, batch_len_r, batch_ent_seq_l1, batch_ent_seq_l2, ent_sep_pos_l, batch_ent_seq_r, ent_sep_pos_r, batch_mention_seq_l1, batch_mention_seq_l2, batch_mention_seq_r, batch_distance_seq_l1, batch_distance_seq_l2, batch_distance_seq_r, batch_first_mention_bit, batch_second_mention_bit, batch_focus_mention_length, batch_other_mention_length, batch_coreference_chain_length, batch_used_bit_seql_1, batch_same_bit_seql_1, batch_used_bit_seql_2, batch_same_bit_seql_2, batch_is_subject_list, batch_is_object_list, batch_label, max_seql_length, max_ent_seql_length, max_ent_seqr_length = \
                ...
                focus_mention_length_n = torch.tensor(batch_focus_mention_length, dtype=torch.float32, requires_grad=False).to(self.device)
                other_mention_length_n = torch.tensor(batch_other_mention_length, dtype=torch.float32, requires_grad=False).to(self.device)
                ...
                # Forward pass: predict scores
                score_l1, score_l2 = model(batch_l1, batch_l2, max_seql_length, rep_r, len_r, batch_ent_seq_l1, batch_ent_seq_l2, ent_sep_pos_l, batch_ent_seq_r, ent_sep_pos_r, max_ent_seql_length, max_ent_seqr_length, rep_mention_l1, rep_mention_l2, rep_mention_r, rep_distance_l1, rep_distance_l2, rep_distance_r, rep_first_mention_bit, rep_second_mention_bit, rep_focus_mention_length, rep_other_mention_length, rep_coreference_chain_length, rep_used_bit_seql_1, rep_same_bit_seql_1, rep_used_bit_seql_2, rep_same_bit_seql_2, rep_is_subject_list, rep_is_object_list, focus_mention_length_n, other_mention_length_n)    
                cost = NN.MarginRankingLoss(margin=margin)(score_l1, score_l2, label)             
                optimizer.zero_grad()            
                cost.backward(retain_graph=True)
                running_loss = running_loss + cost.item()
                optimizer.step()
                
                s = s + 1
            
            print('training loss: %f' % (1.0*running_loss/103))
            sys.stdout.flush()
            
            model_path = "saved_models_lstm_attention/model_epoch_"+str(i+1)+".pt"
            ...
            print ('Accuracy of the trained model on validation set %f' % (1.0*total_correct_dev / total_size_dev))
            sys.stdout.flush()
...
def main():
    ...
    parser.add_argument('--conll_data_dir',
                    type=str,
                    default='E:/mention_predictor_upload/code_preprocess/generate_training_data/output_training_array/conll_padding',
                    help='Directory to put the conll training data.')
    parser.add_argument('--development_data_dir',
                    type=str,
                    default='conll_data/test/',
    ...
    parser.add_argument('--dropout_rate_2',
                    ...
                    default=0.01,
                    help='Dropout rate.')
    parser.add_argument('--dropout_rate_3',
                    type=float,
                    default=0.1,
                    help='Dropout rate.')
    parser.add_argument('--dropout_rate_4',
                    type=float,
                    default=0.1,
                    help='Dropout rate.')
