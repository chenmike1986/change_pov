...
def main():
    ...
    parser.add_argument('--output_file',
                    type=str,
                    default='output_conll_data/training_data_transformer.txt',
                    help='Output file.')
    ...
    for instance in instances:
        ...
        if gold_clusters is not None:
            ...
            for sorted_start_pos in sorted(sorted_start_pos_to_mention.keys()):
                ...
                pre_padding = 0
                post_padding = 0
                if focus_mention_start >= 100 and focus_mention_end + 100 <= len(text) - 1:
                    start_index = focus_mention_start - 100
                    end_index = focus_mention_end + 100
                elif focus_mention_start < 100 and focus_mention_end + 100 <= len(text) - 1:
                    start_index= 0
                    end_index = focus_mention_end + 100
                    pre_padding = 100 - focus_mention_start
                elif focus_mention_start >= 100 and focus_mention_end + 100 > len(text) - 1:
                    start_index = focus_mention_start - 100
                    end_index = len(text) - 1
                    post_padding = 101 - len(text) + focus_mention_end
                elif focus_mention_start < 100 and focus_mention_end + 100 > len(text) - 1:
                    start_index = 0
                    end_index = len(text) - 1
                    pre_padding = 100 - focus_mention_start
                ...
                for i in range(len(pre_mention_text_list)):
                    ...
                    output_token_count = output_token_count + len(pre_mention_sentence_text.split())
                ...
                for i in range(len(post_mention_text_list)):
                    ...
                    output_token_count = output_token_count + len(post_mention_sentence_text.split())
                 
                output_token_count = output_token_count + len(focus_mention_text_span)
